...
import { peek, pop, push } from "@z-react-scheduler/SchedulerMinHeap";
...
} from "@z-react-scheduler/SchedulerProfiling";
...
} from "@z-react-scheduler/SchedulerPriorities";
...
import {
    ...
    frameYieldMs,
    maxYieldMs
} from "@z-react-scheduler/SchedulerFeatureFlags";

import type { PriorityLevel } from "@z-react-scheduler/SchedulerPriorities";

export type SchedulerCallback = ( didTimeout?: boolean ) => SchedulerCallback | null | undefined | void;
...
let taskQueue: Array<SchedulerTask> = [];
let timerQueue: Array<SchedulerTask> = [];
...
function unstable_runWithPriority<T>( priorityLevel: PriorityLevel, eventHandler: () => T ): T {
...
function unstable_wrapCallback<T extends ( ... args: Array<unknown> ) => unknown>( callback: T ): T {
...
function unstable_scheduleCallback( priorityLevel: PriorityLevel, callback: SchedulerCallback, options?: {
    delay: number;
} ): SchedulerTask {
...
function unstable_getFirstCallbackNode(): SchedulerTask | null {
    return peek( taskQueue );
}

function unstable_cancelCallback( task: SchedulerTask ) {
    if ( enableProfiling ) {
        if ( task.isQueued ) {
            const currentTime = getCurrentTime();
...
let taskTimeoutID: ReturnType<typeof setTimeout> | -1 = -1;
