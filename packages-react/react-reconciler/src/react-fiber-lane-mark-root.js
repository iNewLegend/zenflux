...
function markSpawnedDeferredLane(root, spawnedLane, entangledLanes) {
    ...
    root.entanglements[spawnedLaneIndex] |= react_fiber_lane_constants_1.DeferredLane | // If the parent render task suspended, we must also entangle those lanes
        // with the spawned task, so that the deferred task includes all the same
        // updates that the parent task did. We can exclude any lane that is not
        // used for updates (e.g. Offscreen).
        entangledLanes & react_fiber_lane_constants_1.UpdateLanes;
}
function markRootUpdated(root, updateLane) {
    ...
    if (updateLane !== react_fiber_lane_constants_1.IdleLane) {
        root.suspendedLanes = react_fiber_lane_constants_1.NoLanes;
        root.pingedLanes = react_fiber_lane_constants_1.NoLanes;
    }
}
...
function markRootSuspendedInternal(root, suspendedLanes, spawnedLane) {
    ...
    if (spawnedLane !== react_fiber_lane_constants_1.NoLane) {
...
function markRootFinished(root, remainingLanes, spawnedLane) {
    ...
    while (lanes > 0) {
        var index = (0, react_fiber_lane_1.pickArbitraryLaneIndex)(lanes);
        var lane = 1 << index;
        entanglements[index] = react_fiber_lane_constants_1.NoLanes;
        expirationTimes[index] = react_fiber_lane_1.NoTimestamp;
        ...
        if (hiddenUpdatesForLane !== null) {
            ...
            for (var i_1 = 0; i_1 < hiddenUpdatesForLane.length; i_1++) {
                var update = hiddenUpdatesForLane[i_1];
                if (update !== null) {
                    update.lane &= ~react_fiber_lane_constants_1.OffscreenLane;
                }
            }
        }
        lanes &= ~lane;
    ...
    if (spawnedLane !== react_fiber_lane_constants_1.NoLane) {
        ...
        react_fiber_lane_constants_1.NoLanes);
    }
}
