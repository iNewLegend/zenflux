    ...
    if (__DEV__) {
        ...
        if (!react_fiber_hooks_shared_1.ReactFiberHooksFlags.didWarnAboutMismatchedHooksForComponent.has(componentName)) {
            ...
            if (react_fiber_hooks_shared_1.ReactFiberHooksCurrent.hookTypesDev !== null) {
                ...
                for (var i_1 = 0; i_1 <= react_fiber_hooks_shared_1.ReactFiberHooksCurrent.hookTypesUpdateIndexDev; i_1++) {
                    var oldHookName = react_fiber_hooks_shared_1.ReactFiberHooksCurrent.hookTypesDev[i_1];
                    var newHookName = i_1 === react_fiber_hooks_shared_1.ReactFiberHooksCurrent.hookTypesUpdateIndexDev ? currentHookName : oldHookName;
                    var row = "".concat(i_1 + 1, ". ").concat(oldHookName);
    ...
    if (__DEV__) {
        ...
        if (isAsyncFunction) {
            ...
            if (!react_fiber_hooks_shared_1.ReactFiberHooksFlags.didWarnAboutAsyncClientComponent.has(componentName)) {
                ...
                if (root !== null && (0, react_fiber_lane_constants_1.includesBlockingLane)(root, rootRenderLanes)) {
...
function finishRenderingHooks(current, workInProgress, Component) {
    ...
    react_fiber_hooks_shared_1.ReactFiberHooksCurrent.renderLanes = react_fiber_lane_constants_1.NoLanes;
    ...
    if (__DEV__) {
        ...
        if (current !== null && (current.flags & react_fiber_flags_1.StaticMask) !== (workInProgress.flags & react_fiber_flags_1.StaticMask) && // Disable this warning in legacy mode, because legacy Suspense is weird
            // and creates false positives. To make this work in legacy mode, we'd
            // need to mark fibers that commit in an incomplete state, somehow. For
            // now I'll disable the warning that most of the bugs that would trigger
            // it are either exclusive to concurrent mode or exist in both.
            (current.mode & react_type_of_mode_1.ConcurrentMode) !== react_type_of_mode_1.NoMode) {
...
function renderWithHooks(current, workInProgress, Component, props, secondArg, nextRenderLanes) {
    ...
    var shouldDoubleRenderDEV = __DEV__ && react_feature_flags_1.debugRenderPhaseSideEffectsForStrictMode && (workInProgress.mode & react_type_of_mode_1.StrictLegacyMode) !== react_type_of_mode_1.NoMode;
...
function bailoutHooks(current, workInProgress, lanes) {
    ...
    if (__DEV__ && (workInProgress.mode & react_type_of_mode_1.StrictEffectsMode) !== react_type_of_mode_1.NoMode) {
        workInProgress.flags &= ~(react_fiber_flags_1.MountPassiveDev | react_fiber_flags_1.MountLayoutDev | react_fiber_flags_1.Passive | react_fiber_flags_1.Update);
    }
    else {
        workInProgress.flags &= ~(react_fiber_flags_1.Passive | react_fiber_flags_1.Update);
    }
...
function startHostTransition(formFiber, pendingState, callback, formData) {
    ...
    if (formFiber.tag !== react_work_tags_1.HostComponent) {
    ...
    if (formFiber.memoizedState === null) {
        ...
        var newQueue = {
            pending: null,
            lanes: react_fiber_lane_constants_1.NoLanes,
