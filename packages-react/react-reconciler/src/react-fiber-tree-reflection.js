...
function getNearestMountedFiber(fiber) {
    ...
    if (!fiber.alternate) {
        ...
        do {
            node = nextNode;
            if ((node.flags & (react_fiber_flags_1.Placement | react_fiber_flags_1.Hydrating)) !== react_fiber_flags_1.NoFlags) {
    ...
    if (node.tag === react_work_tags_1.HostRoot) {
...
function getSuspenseInstanceFromFiber(fiber) {
    if (fiber.tag === react_work_tags_1.SuspenseComponent) {
...
function getContainerFromFiber(fiber) {
    return fiber.tag === react_work_tags_1.HostRoot ? fiber.stateNode.containerInfo : null;
}
...
function isMounted(component) {
    if (__DEV__) {
        ...
        if (owner !== null && owner.tag === react_work_tags_1.ClassComponent) {
...
function findCurrentFiberUsingSlowPath(fiber) {
    ...
    // unmounted.
    if (a.tag !== react_work_tags_1.HostRoot) {
        throw new Error("Unable to find node on an unmounted component.");
    }
...
function findCurrentHostFiberWithNoPortalsImpl(node) {
    ...
    var child = node.child;
    while (child !== null) {
        if (child.tag !== react_work_tags_1.HostPortal) {
...
function isFiberSuspenseAndTimedOut(fiber) {
    ...
    return fiber.tag === react_work_tags_1.SuspenseComponent && memoizedState !== null && memoizedState.dehydrated === null;
}
