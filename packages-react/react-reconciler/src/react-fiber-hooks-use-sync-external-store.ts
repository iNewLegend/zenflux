...
import { Passive as PassiveEffect, StoreConsistency } from "@zenflux/react-reconciler/src/react-fiber-flags";
import {
...
import { includesBlockingLane, SyncLane } from "@zenflux/react-reconciler/src/react-fiber-lane-constants";
import {
...
export function mountSyncExternalStore<T>( subscribe: ( arg0: () => void ) => () => void, getSnapshot: () => T, getServerSnapshot?: () => T ): T {
    ...
    pushEffect( HookHasEffect | HookPassive, updateStoreInstance.bind( null, fiber, inst, nextSnapshot, getSnapshot ), createEffectInstance(), null );
    return nextSnapshot;
}
...
export function updateSyncExternalStore<T>( subscribe: ( arg0: () => void ) => () => void, getSnapshot: () => T, getServerSnapshot?: () => T ): T {
    ...
    if ( inst.getSnapshot !== getSnapshot || snapshotChanged || // Check if the subscribe function changed. We can save some memory by
        ...
        ReactFiberHooksCurrent.workInProgressHook !== null && ReactFiberHooksCurrent.workInProgressHook.memoizedState.tag & HookHasEffect ) {
        fiber.flags |= PassiveEffect;
        pushEffect( HookHasEffect | HookPassive, updateStoreInstance.bind( null, fiber, inst, nextSnapshot, getSnapshot ), createEffectInstance(), null );
        // Unless we're rendering a blocking lane, schedule a consistency check.
        // Right before committing, we will walk the tree and check if any of the
