...
import { NoLanes, SyncLane } from "@zenflux/react-reconciler/src/react-fiber-lane-constants";
...
import {
    ...
    isExecutionContextRenderActivate
...
import { ConcurrentMode, NoMode, ProfileMode } from "@zenflux/react-reconciler/src/react-type-of-mode";
import {
    ClassComponent,
    ForwardRef,
    FunctionComponent,
...
import type { Lane} from "@zenflux/react-reconciler/src/react-fiber-lane-constants";
import type { Transition } from "@zenflux/react-reconciler/src/react-fiber-tracing-marker-component";
...
export function scheduleUpdateOnFiber( root: FiberRoot, fiber: Fiber, lane: Lane ) {
    ...
    if ( isExecutionContextCommitDeactivate() && root === getWorkInProgressRoot() ) {
        ...
        if ( lane === SyncLane && isExecutionContextEmpty() && ( fiber.mode & ConcurrentMode ) === NoMode ) {
