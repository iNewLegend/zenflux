...
function performSyncWorkOnRoot(root, lanes) {
    ...
    if (root.tag !== react_root_tags_1.LegacyRoot && exitStatus === root_exit_status_1.RootExitStatus.RootErrored) {
...
function flushSyncWorkAcrossRoots_impl(onlyLegacy) {
    ...
    do {
        ...
        while (root !== null) {
            if (onlyLegacy && root.tag !== react_root_tags_1.LegacyRoot) { // Skip non-legacy roots.
            }
            else {
                ...
                if ((0, react_fiber_lane_constants_1.includesSyncLane)(nextLanes)) {
                    ...
                    try {
                        ...
                        // TODO: Added by ZENFLUX - ( Infinity ) loop on error
                        didPerformSomeWork = false;
                    }
                }
            }
    ...
    if (errors !== null) {
        if (errors.length > 1) {
            if (typeof AggregateError === "function") {
                ...
                for (var i_1 = 1; i_1 < errors.length; i_1++) {
                    react_fiber_root_scheduler_shared_1.ReactFiberRootSchedulerShared.scheduleImmediateTask(throwError.bind(null, errors[i_1]));
                }
                throw errors[0];
            }
        }
