...
function processRootScheduleInMicrotask() {
    ...
    while (root !== null) {
        var next = root.next;
        if (react_fiber_root_scheduler_shared_1.ReactFiberRootSchedulerShared.currentEventTransitionLane !== react_fiber_lane_constants_1.NoLane && shouldAttemptEagerTransition()) {
        ...
        if (nextLanes === react_fiber_lane_constants_1.NoLane) {
            ...
            prev = root;
            if ((0, react_fiber_lane_constants_1.includesSyncLane)(nextLanes)) {
                react_fiber_work_on_root_shared_1.ReactFiberWorkOnRootShared.setHavePendingSyncWork();
            }
        }
        root = next;
    }
    react_fiber_root_scheduler_shared_1.ReactFiberRootSchedulerShared.currentEventTransitionLane = react_fiber_lane_constants_1.NoLane;
...
function ensureRootScheduled(root) {
    ...
    if (__DEV__ && ReactCurrentActQueue.isBatchingLegacy && root.tag === react_root_tags_1.LegacyRoot) {
...
function scheduleTaskForRootDuringMicrotask(root, currentTime) {
    ...
    var nextLanes = (0, react_fiber_lane_1.getNextLanes)(root, root === workInProgressRoot ? workInProgressRootRenderLanes : react_fiber_lane_constants_1.NoLanes);
    var existingCallbackNode = root.callbackNode;
    if ( // Check if there's nothing to work on
    nextLanes === react_fiber_lane_constants_1.NoLanes || // If this root is currently suspended and waiting for data to resolve, don't
    ...
    if ((0, react_fiber_lane_constants_1.includesSyncLane)(nextLanes)) {
        ...
        root.callbackPriority = react_fiber_lane_constants_1.SyncLane;
        root.callbackNode = null;
        return react_fiber_lane_constants_1.SyncLane;
    }
    else {
        // We use the highest priority lane to represent the priority of the callback.
        var existingCallbackPriority = root.callbackPriority;
        var newCallbackPriority = (0, react_fiber_lane_constants_1.getHighestPriorityLane)(nextLanes);
