...
function shouldProfile(current) {
    return react_feature_flags_1.enableProfilerTimer &&
        ...
        (current.mode & react_type_of_mode_1.ProfileMode) !== react_type_of_mode_1.NoMode &&
...
function commitAttachRef(finishedWork) {
    ...
    if (ref !== null) {
        ...
        if (react_feature_flags_1.enableScopeAPI && finishedWork.tag === react_work_tags_1.ScopeComponent) {
            instanceToUse = instance;
        }
...
function isHostParent(fiber) {
    return fiber.tag === react_work_tags_1.HostComponent || fiber.tag === react_work_tags_1.HostRoot || (react_feature_flags_1.enableFloat && supportsResources ? fiber.tag === react_work_tags_1.HostHoistable : false) || (react_feature_flags_1.enableHostSingletons && supportsSingletons ? fiber.tag === react_work_tags_1.HostSingleton : false) || fiber.tag === react_work_tags_1.HostPortal;
}
function getHostSibling(fiber) {
    ...
    siblings: while (true) {
        ...
        node = node.sibling;
        while (node.tag !== react_work_tags_1.HostComponent && node.tag !== react_work_tags_1.HostText && (!(react_feature_flags_1.enableHostSingletons && supportsSingletons) ? true : node.tag !== react_work_tags_1.HostSingleton) && node.tag !== react_work_tags_1.DehydratedFragment) {
            // If it is not host node and, we might have a host node inside it.
            // Try to search down until we find one.
            if (node.flags & react_fiber_flags_1.Placement) {
                // If we don't have a child, try the siblings instead.
                continue siblings;
            }
            // If we don't have a child, try the siblings instead.
            // We also skip portals because they are not part of this host tree.
            if (node.child === null || node.tag === react_work_tags_1.HostPortal) {
                continue siblings;
            }
            else {
...
function commitPlacement(finishedWork) {
    ...
    if (react_feature_flags_1.enableHostSingletons && supportsSingletons) {
        if (finishedWork.tag === react_work_tags_1.HostSingleton) {
    ...
    switch (parentFiber.tag) {
        ...
        case react_work_tags_1.HostComponent: {
            ...
            if (parentFiber.flags & react_fiber_flags_1.ContentReset) {
                // Reset the text content of the parent before doing any insertions
                resetTextContent(parent_2);
                // Clear ContentReset from the effect tag
                parentFiber.flags &= ~react_fiber_flags_1.ContentReset;
            }
            var before = getHostSibling(finishedWork);
            // We only have the top Fiber that was inserted but we need to recurse down its
            // children to find all the terminal nodes.
...
function insertOrAppendPlacementNodeIntoContainer(node, before, parent) {
    var tag = node.tag;
    var isHost = tag === react_work_tags_1.HostComponent || tag === react_work_tags_1.HostText;
    if (isHost) {
    ...
    else if (tag === react_work_tags_1.HostPortal || (react_feature_flags_1.enableHostSingletons && supportsSingletons ? tag === react_work_tags_1.HostSingleton : false)) { // If the insertion itself is a portal, then we don't want to traverse
...
// flag instead.
var suspenseyCommitFlag = react_fiber_flags_1.ShouldSuspendCommit;
        ...
        case react_work_tags_1.OffscreenComponent: {
            ...
            if (isHidden) { // Don't suspend in hidden trees
                ...
                if (wasHidden) {
                    ...
                    suspenseyCommitFlag = react_fiber_flags_1.MaySuspendCommit;
                    recursivelyAccumulateSuspenseyCommit(fiber);
