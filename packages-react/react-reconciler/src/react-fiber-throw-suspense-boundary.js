...
function markSuspenseBoundaryShouldCapture(suspenseBoundary, returnFiber, sourceFiber, root, rootRenderLanes) {
    ...
    if ((suspenseBoundary.mode & react_type_of_mode_1.ConcurrentMode) === react_type_of_mode_1.NoMode) {
        ...
        if (suspenseBoundary === returnFiber) {
            ...
            suspenseBoundary.flags |= react_fiber_flags_1.DidCapture;
            sourceFiber.flags |= react_fiber_flags_1.ForceUpdateForLegacySuspense;
            // We're going to commit this fiber even though it didn't complete.
            // But we shouldn't call any lifecycle methods or callbacks. Remove
            // all lifecycle effect tags.
            sourceFiber.flags &= ~(react_fiber_flags_1.LifecycleEffectMask | react_fiber_flags_1.Incomplete);
            if (sourceFiber.tag === react_work_tags_1.ClassComponent) {
                var currentSourceFiber = sourceFiber.alternate;
                if (currentSourceFiber === null) {
                    // This is a new mount. Change the tag so it's not mistaken for a
                    // completed class component. For example, we should not call
                    // componentWillUnmount if it is deleted.
                    sourceFiber.tag = react_work_tags_1.IncompleteClassComponent;
                }
                else {
                    // When we try rendering again, we should not reuse the current fiber,
                    // since it's known to be in an inconsistent state. Use a force update to
                    // prevent a bail out.
                    var update = (0, react_fiber_class_update_queue_1.createUpdate)(react_fiber_lane_constants_1.SyncLane);
                    update.tag = react_fiber_class_update_queue_1.ForceUpdate;
                    (0, react_fiber_class_update_queue_1.enqueueUpdate)(sourceFiber, update, react_fiber_lane_constants_1.SyncLane);
                }
            }
            // The source fiber did not complete. Mark it with Sync priority to
            // indicate that it still has pending work.
            sourceFiber.lanes = (0, react_fiber_lane_1.mergeLanes)(sourceFiber.lanes, react_fiber_lane_constants_1.SyncLane);
        }
