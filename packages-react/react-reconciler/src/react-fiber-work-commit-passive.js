...
function commitTransitionProgress(offscreenFiber) {
    if (react_feature_flags_1.enableTransitionTracing) {
        ...
        if (parent_1 !== null && parent_1.tag === react_work_tags_1.SuspenseComponent && parent_1.memoizedProps.unstable_name) {
            ...
            if (pendingMarkers !== null) {
                pendingMarkers.forEach(function (markerInstance) {
                    ...
                    if (pendingBoundaries !== null && !pendingBoundaries.has(offscreenInstance_1)) {
                        ...
                        if (transitions !== null) {
                            if (markerInstance.tag === react_fiber_tracing_marker_component_1.TransitionTracingMarker && markerName !== null) {
                            ...
                            else if (markerInstance.tag === react_fiber_tracing_marker_component_1.TransitionRoot) {
...
function recursivelyTraverseAtomicPassiveEffects(finishedRoot, parentFiber, committedLanes, committedTransitions) {
    ...
    if (parentFiber.subtreeFlags & react_fiber_flags_1.PassiveMask) {
...
function commitAtomicPassiveEffects(finishedRoot, finishedWork, committedLanes, committedTransitions) {
    ...
    switch (finishedWork.tag) {
        case react_work_tags_1.OffscreenComponent: {
            ...
            if (flags & react_fiber_flags_1.Passive) {
...
function detachFiberAfterEffects(fiber) {
    ...
    // one, too.
    if (fiber.tag === react_work_tags_1.HostComponent) {
...
function commitPassiveUnmountOnFiber(finishedWork) {
    switch (finishedWork.tag) {
        ...
        case react_work_tags_1.SimpleMemoComponent: {
            ...
            if (finishedWork.flags & react_fiber_flags_1.Passive) {
                commitHookPassiveUnmountEffects(finishedWork, finishedWork.return, react_hook_effect_tags_1.Passive | react_hook_effect_tags_1.HasEffect);
            }
            break;
        }
        case react_work_tags_1.OffscreenComponent: {
            ...
            var isHidden = nextState !== null;
            if (isHidden && instance._visibility & react_fiber_activity_component_1.OffscreenPassiveEffectsConnected && ( // For backwards compatibility, don't unmount when a tree suspends. In
            // the future we may change this to unmount after a delay.
            finishedWork.return === null || finishedWork.return.tag !== react_work_tags_1.SuspenseComponent)) {
                // The effects are currently connected. Disconnect them.
                // TODO: Add option or heuristic to delay before disconnecting the
                // effects. Then if the tree reappears before the delay has elapsed, we
                // can skip toggling the effects entirely.
                instance._visibility &= ~react_fiber_activity_component_1.OffscreenPassiveEffectsConnected;
...
function commitPassiveMountOnFiber(finishedRoot, finishedWork, committedLanes, committedTransitions) {
    ...
    switch (finishedWork.tag) {
        ...
        case react_work_tags_1.SimpleMemoComponent: {
            ...
            if (flags & react_fiber_flags_1.Passive) {
                commitHookPassiveMountEffects(finishedWork, react_hook_effect_tags_1.Passive | react_hook_effect_tags_1.HasEffect);
            }
            break;
        }
        case react_work_tags_1.HostRoot: {
        ...
        case react_work_tags_1.OffscreenComponent: {
            ...
            var isHidden = nextState !== null;
            if (isHidden) {
                if (instance._visibility & react_fiber_activity_component_1.OffscreenPassiveEffectsConnected) {
                ...
                if (instance._visibility & react_fiber_activity_component_1.OffscreenPassiveEffectsConnected) {
                    ...
                    // the initial render.
                    instance._visibility |= react_fiber_activity_component_1.OffscreenPassiveEffectsConnected;
                    var includeWorkInProgressEffects = (finishedWork.subtreeFlags & react_fiber_flags_1.PassiveMask) !== react_fiber_flags_1.NoFlags;
...
function commitPassiveUnmountInsideDeletedTreeOnFiber(current, nearestMountedAncestor) {
    switch (current.tag) {
        ...
        case react_work_tags_1.ForwardRef:
        case react_work_tags_1.SimpleMemoComponent: {
            commitHookPassiveUnmountEffects(current, nearestMountedAncestor, react_hook_effect_tags_1.Passive);
            break;
        }
...
function recursivelyTraverseDisconnectPassiveEffects(parentFiber) {
    ...
    if ((parentFiber.flags & react_fiber_flags_1.ChildDeletion) !== react_fiber_flags_1.NoFlags) {
        if (deletions !== null) {
    ...
    if ((parentFiber.flags & react_fiber_flags_1.ChildDeletion) !== react_fiber_flags_1.NoFlags) {
        if (deletions !== null) {
            for (var i_2 = 0; i_2 < deletions.length; i_2++) {
                var childToDelete = deletions[i_2];
                // TODO: Convert this to use recursion
                (0, react_fiber_commit_next_effect_1.setNextEffect)(childToDelete);
                commitPassiveUnmountEffectsInsideOfDeletedTree_begin(childToDelete, parentFiber);
            }
...
function recursivelyTraverseReconnectPassiveEffects(finishedRoot, parentFiber, committedLanes, committedTransitions, includeWorkInProgressEffects) {
    ...
    // node was reused.
    var childShouldIncludeWorkInProgressEffects = includeWorkInProgressEffects && (parentFiber.subtreeFlags & react_fiber_flags_1.PassiveMask) !== react_fiber_flags_1.NoFlags;
...
function commitPassiveEffectDurations(finishedRoot, finishedWork) {
    if (react_feature_flags_1.enableProfilerTimer && react_feature_flags_1.enableProfilerCommitHooks && (0, react_fiber_work_excution_context_1.hasExecutionCommitContext)()) {
        ...
        if ((finishedWork.flags & react_fiber_flags_1.Update) !== react_fiber_flags_1.NoFlags) {
            switch (finishedWork.tag) {
...
function disconnectPassiveEffect(finishedWork) {
    switch (finishedWork.tag) {
        ...
        case react_work_tags_1.SimpleMemoComponent: {
            // TODO: Check PassiveStatic flag
            commitHookPassiveUnmountEffects(finishedWork, finishedWork.return, react_hook_effect_tags_1.Passive);
...
includeWorkInProgressEffects) {
    ...
    switch (finishedWork.tag) {
        ...
        case react_work_tags_1.SimpleMemoComponent: {
            ...
            commitHookPassiveMountEffects(finishedWork, react_hook_effect_tags_1.Passive);
            break;
        }
        ...
        case react_work_tags_1.LegacyHiddenComponent: {
            if (react_feature_flags_1.enableLegacyHidden) {
                recursivelyTraverseReconnectPassiveEffects(finishedRoot, finishedWork, committedLanes, committedTransitions, includeWorkInProgressEffects);
                if (includeWorkInProgressEffects && flags & react_fiber_flags_1.Passive) {
                    // TODO: Pass `current` as argument to this function
                    var current = finishedWork.alternate;
                    var instance = finishedWork.stateNode;
                    commitOffscreenPassiveMountEffects(current, finishedWork, instance);
