        ...
        if (candidateType !== null) {
            ...
            if (family !== undefined) {
                if (staleFamilies.has(family)) {
                    ...
                    if (tag === react_work_tags_1.ClassComponent) {
                        needsRemount = true;
                    }
                    else {
                        needsRender = true;
...
function findChildHostInstancesForFiberShallowly(fiber, hostInstances) {
    if (__DEV__) {
        ...
        while (true) {
            if (node.tag === react_work_tags_1.HostComponent || (react_feature_flags_1.enableFloat ? node.tag === react_work_tags_1.HostHoistable : false) || (react_feature_flags_1.enableHostSingletons && supportsSingletons ? node.tag === react_work_tags_1.HostSingleton : false)) {
                // We got a match.
