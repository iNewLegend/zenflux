...
function dispatchOptimisticSetState(fiber, throwIfDuringRender, queue, action) {
    if (__DEV__) {
        if (ReactCurrentBatchConfig.transition === null) {
            ...
            if ((0, react_fiber_async_action_1.peekEntangledActionLane)() !== react_fiber_lane_constants_1.NoLane) { // There is a pending async action. Don't warn.
            }
            else {
    ...
    var update = {
        ...
        lane: react_fiber_lane_constants_1.SyncLane,
    ...
    if ((0, react_fiber_hooks_infra_1.isRenderPhaseUpdate)(fiber)) {
        ...
        var root = (0, react_fiber_concurrent_updates_1.enqueueConcurrentHookUpdate)(fiber, queue, update, react_fiber_lane_constants_1.SyncLane);
        if (root !== null) {
            // NOTE: The optimistic update implementation assumes that the transition
            // will never be attempted before the optimistic update. This currently
            // holds because the optimistic update is always synchronous. If we ever
            // change that, we'll need to account for this.
            (0, react_fiber_work_schedule_update_1.scheduleUpdateOnFiber)(root, fiber, react_fiber_lane_constants_1.SyncLane); // Optimistic updates are always synchronous, so we don't need to call
            // entangleTransitionUpdate here.
        }
    }
    (0, react_fiber_hooks_infra_1.markUpdateInDevTools)(fiber, react_fiber_lane_constants_1.SyncLane, action);
}
