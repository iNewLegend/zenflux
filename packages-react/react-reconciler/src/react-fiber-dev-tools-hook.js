...
function onCommitRoot(root, eventPriority) {
    if (injectedHook && typeof injectedHook.onCommitFiberRoot === "function") {
        try {
            var didError = (root.current.flags & react_fiber_flags_1.DidCapture) === react_fiber_flags_1.DidCapture;
...
function getLaneLabelMap() {
    if (react_feature_flags_1.enableSchedulingProfiler) {
        var map = new Map();
        var lane = 1;
        for (var index = 0; index < react_fiber_lane_constants_1.TotalLanes; index++) {
            var label = (0, react_fiber_lane_constants_1.getLabelForLane)(lane);
            map.set(lane, label);
            lane *= 2;
        }
        return map;
