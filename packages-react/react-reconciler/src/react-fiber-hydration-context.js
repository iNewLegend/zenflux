    ...
    if (__DEV__) {
        switch (returnFiber.tag) {
            ...
            case react_work_tags_1.HostComponent: {
                var isConcurrentMode = (returnFiber.mode & react_type_of_mode_1.ConcurrentMode) !== react_type_of_mode_1.NoMode;
...
function popHydrationState(fiber) {
    ...
    if (react_feature_flags_1.enableHostSingletons && supportsSingletons) {
        ...
        if (fiber.tag !== react_work_tags_1.HostRoot && fiber.tag !== react_work_tags_1.HostSingleton && !(fiber.tag === react_work_tags_1.HostComponent && (!shouldDeleteUnhydratedTailInstances(fiber.type) || shouldSetTextContent(fiber.type, fiber.memoizedProps)))) {
            shouldClear = true;
        }
    }
    else {
        ...
        if (fiber.tag !== react_work_tags_1.HostRoot && (fiber.tag !== react_work_tags_1.HostComponent || shouldDeleteUnhydratedTailInstances(fiber.type) && !shouldSetTextContent(fiber.type, fiber.memoizedProps))) {
            shouldClear = true;
        }
    }
    if (shouldClear) {
