...
function propagateContextChange_eager(workInProgress, context, renderLanes) {
    ...
    while (fiber !== null) {
        ...
        if (list !== null) {
            ...
            while (dependency !== null) {
                ...
                if (dependency.context === context) {
                    ...
                    if (fiber.tag === react_work_tags_1.ClassComponent) {
        ...
        else if (fiber.tag === react_work_tags_1.ContextProvider) {
            // Don't scan deeper if this is a matching provider
            nextFiber = fiber.type === workInProgress.type ? null : fiber.child;
        }
        else if (fiber.tag === react_work_tags_1.DehydratedFragment) {
    ...
    while (fiber !== null) {
        ...
        if (list !== null) {
            ...
            findChangedDep: while (dep !== null) {
                ...
                var dependency = dep;
                var consumer = fiber;
                for (var i_1 = 0; i_1 < contexts.length; i_1++) {
                    var context = contexts[i_1];
                    // Check if the context matches.
...
function propagateParentContextChanges(current, workInProgress, renderLanes, forcePropagateEntireTree) {
    ...
    while (parent !== null) {
        if (!isInsidePropagationBailout) {
            if ((parent.flags & react_fiber_flags_1.NeedsPropagation) !== react_fiber_flags_1.NoFlags) {
                isInsidePropagationBailout = true;
            }
            else if ((parent.flags & react_fiber_flags_1.DidPropagateContext) !== react_fiber_flags_1.NoFlags) {
                break;
            }
        }
        if (parent.tag === react_work_tags_1.ContextProvider) {
    ...
    workInProgress.flags |= react_fiber_flags_1.DidPropagateContext;
}
...
function readContextForConsumer(consumer, context) {
    ...
    if (lastFullyObservedContext === context) { // Nothing to do. We already observe everything in this context.
        ...
        if (lastContextDependency === null) {
            ...
            if (react_feature_flags_1.enableLazyContextPropagation) {
                consumer.flags |= react_fiber_flags_1.NeedsPropagation;
            }
        }
        else {
