...
import { debugRenderPhaseSideEffectsForStrictMode } from "@zenflux/react-shared/src/react-feature-flags";
...
import { Callback, DidCapture, ShouldCapture, Visibility } from "@zenflux/react-reconciler/src/react-fiber-flags";
import {
    intersectLanes,
    isSubsetOfLanes,
    mergeLanes,
...
import { isTransitionLane, NoLane, NoLanes, OffscreenLane } from "@zenflux/react-reconciler/src/react-fiber-lane-constants";
import {
...
import { StrictLegacyMode } from "@zenflux/react-reconciler/src/react-type-of-mode";
...
export type SharedQueue<State, UpdateType = Update<State>> = {
...
let didWarnUpdateInsideUpdate: boolean;
let currentlyProcessingQueue: SharedQueue<any> | null | undefined;
...
export function enqueueUpdate<State>( fiber: Fiber, update: Update<State>, lane: Lane ): FiberRoot | null {
    ...
    if ( __DEV__ ) {
        if ( currentlyProcessingQueue === sharedQueue && ! didWarnUpdateInsideUpdate ) {
            const componentName = reactGetComponentNameFromFiber( fiber );
...
export function processUpdateQueue<State>( workInProgress: Fiber, props: any, instance: any, renderLanes: Lanes ): void {
    ...
    if ( firstBaseUpdate !== null ) {
        ...
        let newBaseState: null | State = null;
        let newFirstBaseUpdate: null | Update<State> = null;
