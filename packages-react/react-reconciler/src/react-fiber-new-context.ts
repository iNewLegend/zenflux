...
import { REACT_SERVER_CONTEXT_DEFAULT_VALUE_NOT_LOADED } from "@zenflux/react-shared/src/react-symbols";
...
import { createUpdate, ForceUpdate } from "@zenflux/react-reconciler/src/react-fiber-class-update-queue";

import { ClassComponent, ContextProvider, DehydratedFragment } from "@zenflux/react-reconciler/src/react-work-tags";
import { DidPropagateContext, NeedsPropagation, NoFlags } from "@zenflux/react-reconciler/src/react-fiber-flags";

import { getHostTransitionProvider, HostTransitionContext } from "@zenflux/react-reconciler/src/react-fiber-host-context";
...
} from "@zenflux/react-reconciler/src/react-fiber-new-context-disallowed-in-dev";

import type { TransitionStatus } from "@zenflux/react-reconciler/src/react-fiber-config";
import type { SharedQueue } from "@zenflux/react-reconciler/src/react-fiber-class-update-queue";
...
import type { ContextDependency, Dependencies, Fiber } from "@zenflux/react-reconciler/src/react-internal-types";
...
let rendererCursorDEV: StackCursor<Record<string, any> | null>;

if ( __DEV__ ) {
    rendererCursorDEV = createCursor( null );
}

let renderer2CursorDEV: StackCursor<Record<string, any> | null>;

if ( __DEV__ ) {
    renderer2CursorDEV = createCursor( null );
}

let rendererSigil: Record<string, any> | null | undefined;
...
function propagateParentContextChanges( current: Fiber, workInProgress: Fiber, renderLanes: Lanes, forcePropagateEntireTree: boolean ) {
    ...
    let contexts: ReactContext<any>[] | null = null;
    ...
    while ( parent !== null ) {
        ...
        } else if ( enableFormActions && enableAsyncActions && parent === getHostTransitionProvider() ) {
...
export function readContextDuringReconcilation<T>( consumer: Fiber, context: ReactContext<T>, renderLanes: Lanes ): T {
