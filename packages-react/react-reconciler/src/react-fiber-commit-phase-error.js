...
function captureCommitPhaseErrorOnRoot(rootFiber, sourceFiber, error) {
    ...
    var update = (0, react_fiber_throw_error_update_1.createRootErrorUpdate)(rootFiber, errorInfo, react_fiber_lane_constants_1.SyncLane);
    var root = (0, react_fiber_class_update_queue_1.enqueueUpdate)(rootFiber, update, react_fiber_lane_constants_1.SyncLane);
    if (root !== null) {
        (0, react_fiber_lane_mark_root_1.markRootUpdated)(root, react_fiber_lane_constants_1.SyncLane);
...
function captureCommitPhaseError(sourceFiber, nearestMountedAncestor, error) {
    ...
    if (sourceFiber.tag === react_work_tags_1.HostRoot) {
    ...
    while (fiber !== null) {
        if (fiber.tag === react_work_tags_1.HostRoot) {
            captureCommitPhaseErrorOnRoot(fiber, sourceFiber, error);
            return;
        }
        else if (fiber.tag === react_work_tags_1.ClassComponent) {
            var ctor = fiber.type;
            ...
            if (typeof ctor.getDerivedStateFromError === "function" || typeof instance.componentDidCatch === "function" && !(0, react_fiber_work_legacy_error_boundary_1.isAlreadyFailedLegacyErrorBoundary)(instance)) {
                var errorInfo = (0, react_captured_value_1.createCapturedValueAtFiber)(error, sourceFiber);
                var update = (0, react_fiber_throw_error_update_1.createClassErrorUpdate)(fiber, errorInfo, react_fiber_lane_constants_1.SyncLane);
                var root = (0, react_fiber_class_update_queue_1.enqueueUpdate)(fiber, update, react_fiber_lane_constants_1.SyncLane);
                if (root !== null) {
                    (0, react_fiber_lane_mark_root_1.markRootUpdated)(root, react_fiber_lane_constants_1.SyncLane);
                    react_fiber_root_scheduler_shared_1.ReactFiberRootSchedulerShared.ensureRootScheduled(root);
                }
