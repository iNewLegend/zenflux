...
import { hasFailedErrorBoundary, hasSpecificFailedErrorBoundarySafe } from "@zenflux/react-reconciler/src/react-fiber-hot-reloading-error-boundray";
...
function findChildHostInstancesForFiberShallowly( fiber: Fiber, hostInstances: Set<Instance> ): boolean {
    if ( __DEV__ ) {
        ...
        while ( true ) {
            if ( node.tag === HostComponent || ( enableFloat ? node.tag === HostHoistable : false ) || ( enableHostSingletons && supportsSingletons ? node.tag === HostSingleton : false ) ) {
                // We got a match.
