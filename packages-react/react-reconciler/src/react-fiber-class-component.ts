...
import { get as getInstance, set as setInstance } from "@zenflux/react-shared/src/react-instance-map";
import { REACT_CONTEXT_TYPE, REACT_PROVIDER_TYPE } from "@zenflux/react-shared/src/react-symbols";
import shallowEqual from "@zenflux/react-shared/src/shallow-equal";

import { logForceUpdateScheduled, logStateUpdateScheduled } from "@zenflux/react-reconciler/src/react-debug-tracing";
...
import { LayoutStatic, MountLayoutDev, Snapshot, Update } from "@zenflux/react-reconciler/src/react-fiber-flags";
...
import { isMounted } from "@zenflux/react-reconciler/src/react-fiber-tree-reflection";
import ReactStrictModeWarnings from "@zenflux/react-reconciler/src/react-strict-mode-warnings";

import { DebugTracingMode, NoMode, StrictEffectsMode, StrictLegacyMode } from "@zenflux/react-reconciler/src/react-type-of-mode";
...
let didWarnAboutStateAssignmentForComponent: Set<string>;
let didWarnAboutUninitializedState: Set<string>;
let didWarnAboutGetSnapshotBeforeUpdateWithoutDidUpdate: Set<string>;
let didWarnAboutLegacyLifecyclesAndDerivedState: Set<string>;
let didWarnAboutUndefinedDerivedState: Set<string>;
let didWarnAboutDirectlyAssigningPropsToState: Set<string>;
let didWarnAboutContextTypeAndContextTypes: Set<string>;
let didWarnAboutInvalidateContextType: Set<string>;
let didWarnOnInvalidCallback: Set<string>;
...
function warnOnInvalidCallback( callback: unknown, callerName: string ) {
    if ( __DEV__ ) {
...
const classComponentUpdater = {
    ...
    enqueueSetState( inst: any, payload: any, callback: Function | null ) {
    ...
    // $FlowFixMe[missing-local-annot]
    enqueueForceUpdate( inst: any, callback: Function | null ) {
        const fiber = getInstance( inst );
        const lane = requestUpdateLane( fiber );
        const update = createUpdate( lane );
...
function checkShouldComponentUpdate( workInProgress: Fiber, ctor: any, oldProps: any, newProps: any, oldState: any, newState: any, nextContext: any ) {
...
function callComponentWillMount( workInProgress: Fiber, instance: any ) {
    ...
    if ( oldState !== instance.state ) {
        if ( __DEV__ ) {
            console.error( "%s.componentWillMount(): Assigning directly to this.state is " + "deprecated (except inside a component's " + "constructor). Use setState instead.", reactGetComponentNameFromFiber( workInProgress ) || "Component" );
        }
...
function callComponentWillReceiveProps( workInProgress: Fiber, instance: any, newProps: any, nextContext: any ) {
    const oldState = instance.state;
    ...
    if ( instance.state !== oldState ) {
        if ( __DEV__ ) {
            const componentName = reactGetComponentNameFromFiber( workInProgress ) || "Component";
