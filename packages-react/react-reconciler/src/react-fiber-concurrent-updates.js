...
var concurrentlyUpdatedLanes = react_fiber_lane_constants_1.NoLanes;
function finishQueueingConcurrentUpdates() {
    var endIndex = concurrentQueuesIndex;
    concurrentQueuesIndex = 0;
    concurrentlyUpdatedLanes = react_fiber_lane_constants_1.NoLanes;
    var i = 0;
    while (i < endIndex) {
        ...
        if (lane !== react_fiber_lane_constants_1.NoLane) {
...
function enqueueConcurrentHookUpdateAndEagerlyBailout(fiber, queue, update) {
    ...
    var lane = react_fiber_lane_constants_1.NoLane;
    var concurrentQueue = queue;
    var concurrentUpdate = update;
    enqueueUpdate(fiber, concurrentQueue, concurrentUpdate, lane);
...
function markUpdateLaneFromFiberToRoot(sourceFiber, update, lane) {
    ...
    while (parent !== null) {
        ...
        if (parent.tag === react_work_tags_1.OffscreenComponent) {
            ...
            if (offscreenInstance !== null && !(offscreenInstance._visibility & react_fiber_activity_component_1.OffscreenVisible)) {
                isHidden = true;
            }
        }
        node = parent;
    ...
    if (isHidden && update !== null && node.tag === react_work_tags_1.HostRoot) {
...
function getRootForUpdatedFiber(sourceFiber) {
    ...
    return node.tag === react_work_tags_1.HostRoot ? node.stateNode : null;
}
...
function warnAboutUpdateOnNotYetMountedFiberInDEV(fiber) {
    if (__DEV__) {
        ...
        if (!(fiber.mode & react_type_of_mode_1.ConcurrentMode)) {
            return;
        }
        var tag = fiber.tag;
        if (tag !== react_work_tags_1.IndeterminateComponent && tag !== react_work_tags_1.HostRoot && tag !== react_work_tags_1.ClassComponent && tag !== react_work_tags_1.FunctionComponent && tag !== react_work_tags_1.ForwardRef && tag !== react_work_tags_1.MemoComponent && tag !== react_work_tags_1.SimpleMemoComponent) {
...
function detectUpdateOnUnmountedFiber(sourceFiber, parent) {
    if (__DEV__) {
        ...
        if (alternate === null && (parent.flags & (react_fiber_flags_1.Placement | react_fiber_flags_1.Hydrating)) !== react_fiber_flags_1.NoFlags) {
