...
function matchSelector(fiber, selector) {
    ...
    switch (selector.$$typeof) {
        ...
        case ROLE_TYPE:
            if (tag === react_work_tags_1.HostComponent || tag === react_work_tags_1.HostHoistable || tag === react_work_tags_1.HostSingleton) {
            ...
            break;
        case TEXT_TYPE:
            if (tag === react_work_tags_1.HostComponent || tag === react_work_tags_1.HostText || tag === react_work_tags_1.HostHoistable || tag === react_work_tags_1.HostSingleton) {
...
function findPaths(root, selectors) {
    ...
    while (index < stack.length) {
        ...
        if ((tag === react_work_tags_1.HostComponent || tag === react_work_tags_1.HostHoistable || tag === react_work_tags_1.HostSingleton) && isHiddenSubtree(fiber)) {
            continue;
        }
        else {
...
function getFindAllNodesFailureDescription(hostRoot, selectors) {
    ...
    if (maxSelectorIndex < selectors.length) {
        var unmatchedNames = [];
        for (var i_1 = maxSelectorIndex; i_1 < selectors.length; i_1++) {
            unmatchedNames.push(selectorToString(selectors[i_1]));
        }
...
function findBoundingRects(hostRoot, selectors) {
    ...
    var boundingRects = [];
    for (var i_2 = 0; i_2 < instanceRoots.length; i_2++) {
        boundingRects.push(getBoundingRect(instanceRoots[i_2]));
    }
    for (var i_3 = boundingRects.length - 1; i_3 > 0; i_3--) {
        var targetRect = boundingRects[i_3];
        var targetLeft = targetRect.x;
        var targetRight = targetLeft + targetRect.width;
        var targetTop = targetRect.y;
        var targetBottom = targetTop + targetRect.height;
        for (var j = i_3 - 1; j >= 0; j--) {
            if (i_3 !== j) {
