import checkPropTypes from "@zenflux/react-shared/src/check-prop-types";
...
import { getIteratorFn, REACT_LAZY_TYPE } from "@zenflux/react-shared/src/react-symbols";
import shallowEqual from "@zenflux/react-shared/src/shallow-equal";

import { createCapturedValue, createCapturedValueAtFiber } from "@zenflux/react-reconciler/src/react-captured-value";
import { getCurrentFiberOwnerNameInDevOrNull, setIsRendering } from "@zenflux/react-reconciler/src/react-current-fiber";
import {
    createFiberFromFragment,
} from "@zenflux/react-reconciler/src/react-fiber";
import { OffscreenDetached } from "@zenflux/react-reconciler/src/react-fiber-activity-component";
import { CacheContext } from "@zenflux/react-reconciler/src/react-fiber-cache-component";
import { pushCacheProvider } from "@zenflux/react-reconciler/src/react-fiber-cache-component-provider";
import {
    cloneChildFibers,
    mountChildFibers,
    reconcileChildFibers
} from "@zenflux/react-reconciler/src/react-fiber-child";
import {
    adoptClassInstance,
    ...
    updateClassInstance
} from "@zenflux/react-reconciler/src/react-fiber-class-component";
import {
    cloneUpdateQueue,
...
import { isSimpleFunctionComponent, resolveLazyComponentTag } from "@zenflux/react-reconciler/src/react-fiber-create-utils";
import {
...
import { pushHiddenContext, reuseHiddenContextOnStack } from "@zenflux/react-reconciler/src/react-fiber-hidden-context";
import {
    bailoutHooks,
...
} from "@zenflux/react-reconciler/src/react-fiber-hooks";
import {
...
import {
    claimHydratableSingleton, tryToClaimNextHydratableInstance,
    tryToClaimNextHydratableSuspenseInstance,
    tryToClaimNextHydratableTextInstance
} from "@zenflux/react-reconciler/src/react-fiber-hydration-context-try-claim";
...
import {
    ...
    SomeRetryLane,
    SyncLane
} from "@zenflux/react-reconciler/src/react-fiber-lane-constants";
...
} from "@zenflux/react-reconciler/src/react-fiber-new-context";
...
import { createClassErrorUpdate } from "@zenflux/react-reconciler/src/react-fiber-throw-error-update";
import {
    getMarkerInstances,
    pushMarkerInstance,
    pushRootMarkerInstance,
    TransitionTracingMarker
} from "@zenflux/react-reconciler/src/react-fiber-tracing-marker-component";
import {
...
} from "@zenflux/react-reconciler/src/react-fiber-transition";
import {
    getForksAtLevel,
    isForkedChild,
    pushMaterializedTreeId,
    pushTreeId
} from "@zenflux/react-reconciler/src/react-fiber-tree-context";
import {
    getWorkInProgressRoot,
    markSkippedUpdateLanes
} from "@zenflux/react-reconciler/src/react-fiber-work-in-progress";
import { createWorkInProgress } from "@zenflux/react-reconciler/src/react-fiber-work-in-progress-ex";
import {
    didWorkInProgressReceiveUpdate,
...
import { retryDehydratedSuspenseBoundary } from "@zenflux/react-reconciler/src/react-fiber-work-retry-boundary";
...
import {
    clearWarningAboutReassigningProps,
    didWarnAboutReassigningProps,
    markWarningAboutReassigningProps
} from "@zenflux/react-reconciler/src/react-fiber-work-warn-about-reassigning-props";
import reactGetComponentNameFromFiber from "@zenflux/react-reconciler/src/react-get-component-name-from-fiber";
import { stopProfilerTimerIfRunning } from "@zenflux/react-reconciler/src/react-profile-timer";
import ReactStrictModeWarnings from "@zenflux/react-reconciler/src/react-strict-mode-warnings";
import { ConcurrentMode, NoMode, ProfileMode, StrictLegacyMode } from "@zenflux/react-reconciler/src/react-type-of-mode";
import {
    CacheComponent,
    ClassComponent,
    ContextConsumer,
...
import { shouldSuspend } from "@zenflux/react-reconciler/src/react-fiber-should-suspend";

import { shouldError } from "@zenflux/react-reconciler/src/react-fiber-should-error";
...
import type {
    ...
    OffscreenQueue,
    OffscreenState
} from "@zenflux/react-reconciler/src/react-fiber-activity-component";
import type {
    Cache,
    CacheComponentState,
    SpawnedCachePool
} from "@zenflux/react-reconciler/src/react-fiber-cache-component";
import type { UpdateQueue } from "@zenflux/react-reconciler/src/react-fiber-class-update-queue";
import type { SuspenseInstance, TransitionStatus } from "@zenflux/react-reconciler/src/react-fiber-config";
...
import type { SuspenseContext } from "@zenflux/react-reconciler/src/react-fiber-suspense-context";
...
import type { LazyComponent as LazyComponentType } from "@zenflux/react-shared/src/lazy-component";
...
if ( __DEV__ ) {
    didWarnAboutBadClass = ( {} as Record<string, boolean> );
    didWarnAboutModulePatternComponent = ( {} as Record<string, boolean> );
    didWarnAboutContextTypeOnFunctionComponent = ( {} as Record<string, boolean> );
    didWarnAboutGetDerivedStateOnFunctionComponent = ( {} as Record<string, boolean> );
    didWarnAboutFunctionRefs = ( {} as Record<string, boolean> );

    clearWarningAboutReassigningProps();

    didWarnAboutRevealOrder = ( {} as { [key in never]?: boolean } );
    didWarnAboutTailOptions = ( {} as Record<string, boolean> );
    didWarnAboutDefaultPropsOnFunctionComponent = ( {} as Record<string, boolean> );
}
...
function updateForwardRef( current: Fiber | null, workInProgress: Fiber, Component: any, nextProps: any, renderLanes: Lanes ) {
    ...
    if ( current !== null && ! didWorkInProgressReceiveUpdate() ) {
        bailoutHooks( current, workInProgress, renderLanes );
        return bailoutOnAlreadyFinishedWork( current, workInProgress, renderLanes );
    }

    if ( isHydrating() && hasId ) {
...
function updateOffscreenComponent( current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes ) {
    ...
    if ( nextProps.mode === "hidden" || enableLegacyHidden && nextProps.mode === "unstable-defer-without-hiding" || nextIsDetached ) {
        ...
        if ( prevState !== null ) {
            ...
            let prevCachePool: typeof prevState.cachePool = null;
...
export function replayFunctionComponent( current: Fiber | null, workInProgress: Fiber, nextProps: any, Component: any, secondArg: any, renderLanes: Lanes ): Fiber | null {
    ...
    if ( __DEV__ ) {
        ...
        if ( shouldUpdate && inst.props !== nextProps ) {
            if ( ! didWarnAboutReassigningProps() ) {
                console.error( "It looks like %s is reassigning its own `this.props` while rendering. " + "This is not supported and can lead to confusing bugs.", reactGetComponentNameFromFiber( workInProgress ) || "a component" );
            }
...
function updateHostRoot( current: null | Fiber, workInProgress: Fiber, renderLanes: Lanes ) {
    ...
    if ( supportsHydration && prevState.isDehydrated ) {
        ...
        if ( workInProgress.flags & ForceClientRender ) {
            ...
            const recoverableError = createCapturedValueAtFiber<unknown>( new Error( "There was an error while hydrating. Because the error happened outside " + "of a Suspense boundary, the entire root will switch to " + "client rendering." ), workInProgress );
            return mountHostRootWithoutHydrating( current, workInProgress, nextChildren, renderLanes, recoverableError );
        } else if ( nextChildren !== prevChildren ) {
            const recoverableError = createCapturedValueAtFiber<unknown>( new Error( "This root received an early update, before anything was able " + "hydrate. Switched the entire root to client rendering." ), workInProgress );
            return mountHostRootWithoutHydrating( current, workInProgress, nextChildren, renderLanes, recoverableError );
        } else {
    ...
    if ( enableFormActions && enableAsyncActions ) {
        ...
        if ( memoizedState !== null ) {
            ...
            if ( enableLazyContextPropagation ) {// In the lazy propagation implementation, we don't scan for matching
                ...
                if ( didWorkInProgressReceiveUpdate() ) {
                    if ( current !== null ) {
...
function mountLazyComponent( _current: null | Fiber, workInProgress: Fiber, elementType: any, renderLanes: Lanes ) {
...
function mountIncompleteClassComponent( _current: null | Fiber, workInProgress: Fiber, Component: any, nextProps: any, renderLanes: Lanes ) {
    resetSuspendedCurrentOnMountInLegacyMode( _current, workInProgress );
...
function mountIndeterminateComponent( _current: null | Fiber, workInProgress: Fiber, Component: any, renderLanes: Lanes ) {
    resetSuspendedCurrentOnMountInLegacyMode( _current, workInProgress );
    const props = workInProgress.pendingProps;
    let context;
...
function mountSuspenseFallbackChildren( workInProgress: Fiber, primaryChildren: any, fallbackChildren: any, renderLanes: Lanes ) {
    const mode = workInProgress.mode;
...
function updateSuspensePrimaryChildren( current: Fiber, workInProgress: Fiber, primaryChildren: any, renderLanes: Lanes ) {
...
function updateSuspenseFallbackChildren( current: Fiber, workInProgress: Fiber, primaryChildren: any, fallbackChildren: any, renderLanes: Lanes ) {
    const mode = workInProgress.mode;
    const currentPrimaryChildFragment: Fiber = ( current.child as any );
    const currentFallbackChildFragment: Fiber | null = currentPrimaryChildFragment.sibling;
    const primaryChildProps: OffscreenProps = {
...
function mountSuspenseFallbackAfterRetryWithoutHydrating( current: Fiber, workInProgress: Fiber, primaryChildren: any, fallbackChildren: any, renderLanes: Lanes ) {
    ...
    if ( ! didSuspend ) {
        ...
        if ( isSuspenseInstanceFallback( suspenseInstance ) ) {
            ...
            if ( ! enablePostpone || digest !== "POSTPONE" ) {
                ...
                capturedValue = createCapturedValue<any>( error, digest, stack );
            }
        ...
        if ( didWorkInProgressReceiveUpdate() || hasContextChanged ) {
        ...
        if ( workInProgress.flags & ForceClientRender ) {
            ...
            pushPrimaryTreeSuspenseHandler( workInProgress );
            workInProgress.flags &= ~ForceClientRender;
            const capturedValue = createCapturedValue<Error>( new Error( "There was an error while hydrating this Suspense boundary. " + "Switched to client rendering." ) );
            return retrySuspenseComponentWithoutHydrating( current, workInProgress, renderLanes, capturedValue );
...
function validateSuspenseListChildren( children: any[] | false, revealOrder: SuspenseListRevealOrder ) {
    if ( __DEV__ ) {
...
function initSuspenseListRenderState( workInProgress: Fiber, isBackwards: boolean, tail: null | Fiber, lastContentRow: null | Fiber, tailMode: SuspenseListTailMode | undefined ): void {
...
function updateScopeComponent( current: null | Fiber, workInProgress: Fiber, renderLanes: Lanes ) {
    const nextProps = workInProgress.pendingProps;
    const nextChildren = nextProps.children;
    reconcileChildren( current, workInProgress, nextChildren, renderLanes );
    return workInProgress.child;
...
function resetSuspendedCurrentOnMountInLegacyMode( current: null | Fiber, workInProgress: Fiber ) {
    if ( ( workInProgress.mode & ConcurrentMode ) === NoMode ) {
        if ( current !== null ) {
...
function beginWork( current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes ): Fiber | null {
    if ( __DEV__ ) {
        // Added by zenflux
        /*if ( "undefined" === typeof workInProgress ) {
            console.error( "beginWork: workInProgress is undefined, current:", current );

            return null;
        } else  End Add by ZenFlux */ if ( workInProgress._debugNeedsRemount && current !== null ) {
    ...
    if ( current !== null ) {
        ...
        if ( isHydrating() && isForkedChild( workInProgress ) ) {
