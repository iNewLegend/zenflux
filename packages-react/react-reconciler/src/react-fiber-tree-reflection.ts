...
import type { Container, SuspenseInstance } from "@zenflux/react-reconciler/src/react-fiber-config";
...
export function isMounted( component: Component ): boolean {
    if ( __DEV__ ) {
...
function findCurrentHostFiberImpl( node: Fiber ): Fiber | null {
    ...
    if ( tag === HostComponent || ( enableFloat ? tag === HostHoistable : false ) || ( enableHostSingletons ? tag === HostSingleton : false ) || tag === HostText ) {
        return node;
    }
