...
import {
    ...
    completeSegment
} from "@zenflux/react-dom-bindings/src/server/fizz-instruction-set/ReactDOMFizzInstructionSetExternalRuntime";
...
function handleNode( node_: Node ) {
    ...
    if ( dataset[ "rxi" ] != null ) {
        clientRenderBoundary( dataset[ "bid" ], dataset[ "dgst" ], dataset[ "msg" ], dataset[ "stck" ] );
        node.remove();
    } else if ( dataset[ "rri" ] != null ) {
        // Convert styles here, since its type is Array<Array<string>>
        completeBoundaryWithStyles( dataset[ "bid" ], dataset[ "sid" ], JSON.parse( dataset[ "sty" ] ) );
        node.remove();
