...
exports.listenToFormSubmissionsForReplaying = exports.completeSegment = exports.completeBoundary = exports.clientRenderBoundary = exports.SUSPENSE_FALLBACK_START_DATA = exports.SUSPENSE_PENDING_START_DATA = exports.SUSPENSE_END_DATA = exports.SUSPENSE_START_DATA = exports.COMMENT_NODE = void 0;
...
exports.SUSPENSE_START_DATA = "$";
exports.SUSPENSE_END_DATA = "/$";
exports.SUSPENSE_PENDING_START_DATA = "$?";
exports.SUSPENSE_FALLBACK_START_DATA = "$!";
...
function clientRenderBoundary(suspenseBoundaryID, errorDigest, errorMsg, errorComponentStack) {
    ...
    // @ts-ignore
    suspenseNode.data = exports.SUSPENSE_FALLBACK_START_DATA;
    // assign error metadata to first sibling
    var dataset = suspenseIdNode.dataset;
    if (errorDigest)
        dataset["dgst"] = errorDigest;
    if (errorMsg)
        dataset["msg"] = errorMsg;
    if (errorComponentStack)
        dataset["stck"] = errorComponentStack;
    // Tell React to retry it if the parent already hydrated.
    if (suspenseNode["_reactRetry"]) {
        suspenseNode["_reactRetry"]();
    }
}
exports.clientRenderBoundary = clientRenderBoundary;
function completeBoundary(suspenseBoundaryID, contentID, errorDigest) {
    ...
    if (!errorDigest) {
        ...
        var parentInstance = suspenseNode.parentNode;
        var node = suspenseNode.nextSibling;
        var depth = 0;
        do {
            if (node && node.nodeType === exports.COMMENT_NODE) {
                // @ts-ignore
                var data = node.data;
                if (data === exports.SUSPENSE_END_DATA) {
                    if (depth === 0) {
                        break;
                    }
                    else {
                ...
                else if (data === exports.SUSPENSE_START_DATA || data === exports.SUSPENSE_PENDING_START_DATA || data === exports.SUSPENSE_FALLBACK_START_DATA) {
                    depth++;
                }
            }
        ...
        var endOfBoundary = node;
        // Insert all the children from the contentNode between the start and end of suspense boundary.
        while (contentNode.firstChild) {
            parentInstance.insertBefore(contentNode.firstChild, endOfBoundary);
        }
        // @ts-ignore
        suspenseNode.data = exports.SUSPENSE_START_DATA;
    }
    else {
        // @ts-ignore
        suspenseNode.data = exports.SUSPENSE_FALLBACK_START_DATA;
...
exports.completeSegment = completeSegment;
...
function listenToFormSubmissionsForReplaying() {
...
exports.listenToFormSubmissionsForReplaying = listenToFormSubmissionsForReplaying;
