...
import { flushResources, getRenderState, getResumableState, resolveRequest } from "react-server/src/ReactFizzServer";
...
} from "@zenflux/react-shared/src/check-string-coercion";
...
} from "@zenflux/react-dom-bindings/src/server/fizz-instruction-set/ReactDOMFizzInstructionSetInlineCodeStrings";

import escapeTextForBrowser from "@zenflux/react-dom-bindings/src/server/escapeTextForBrowser";

import type { ReactCustomFormAction, ReactNodeList } from "@zenflux/react-shared/src/react-types";
...
export type RenderState = {
    ...
    stylesToHoist: boolean; // We allow the legacy renderer to extend this object.
...
export type ResumableState = {
    ...
    unknownResources: Record<string, Record<string, Preloaded>>;
    dnsResources: Record<string, Exists>;
    connectResources: {
        default: Record<string, Exists>;
        anonymous: Record<string, Exists>;
        credentials: Record<string, Exists>;
    };
    imageResources: Record<string, Preloaded>;
    styleResources: Record<string, Exists | Preloaded | PreloadedWithCredentials>;
    scriptResources: Record<string, Exists | Preloaded | PreloadedWithCredentials>;
    moduleUnknownResources: Record<string, Record<string, Preloaded>>;
    moduleScriptResources: Record<string, Exists | Preloaded | PreloadedWithCredentials>;
};
...
const scriptReplacer = ( match: string, prefix: string, s: string, suffix: string ) => `${ prefix }${ s === "s" ? "\\u0073" : "\\u0053" }${ suffix }`;
...
export function createRenderState( resumableState: ResumableState, nonce: string | void, bootstrapScriptContent: string | void, bootstrapScripts: ReadonlyArray<string | BootstrapScriptDescriptor> | void, bootstrapModules: ReadonlyArray<string | BootstrapScriptDescriptor> | void, externalRuntimeConfig: string | BootstrapScriptDescriptor | void, importMap: ImportMap | void ): RenderState {
...
export function resumeRenderState( resumableState: ResumableState, nonce: string | void ): RenderState {
    return createRenderState( resumableState, nonce, // These should have already been flushed in the prerender.
        undefined, undefined, undefined, undefined, undefined );
}

export function createResumableState( identifierPrefix: string | void, externalRuntimeConfig: string | BootstrapScriptDescriptor | void ): ResumableState {
...
export function resetResumableState( resumableState: ResumableState, renderState: RenderState ): void {
...
export type FormatContext = {
    ...
    // root/svg/html/mathml/table
    selectedValue: null | string | Array<string>;
    // the selected value(s) inside a <select>, or null outside <select>
    tagScope: number;
};

function createFormatContext( insertionMode: InsertionMode, selectedValue: null | string, tagScope: number ): FormatContext {
    return {
        insertionMode,
        selectedValue,
        tagScope
...
export function getChildFormatContext( parentContext: FormatContext, type: string, props: Record<string, any> ): FormatContext {
    switch ( type ) {
        case "noscript":
...
export function makeId( resumableState: ResumableState, treeId: string, localId: number ): string {
    const idPrefix = resumableState.idPrefix;
...
function pushStyleAttribute( target: Array<Chunk | PrecomputedChunk>, style: Record<string, any> ): void {
...
function pushBooleanAttribute( target: Array<Chunk | PrecomputedChunk>, name: string, value: string | boolean | number | ( ( ... args: Array<any> ) => any ) | Record<string, any> ) // not null or undefined
    : void {
...
function pushStringAttribute( target: Array<Chunk | PrecomputedChunk>, name: string, value: string | boolean | number | ( ( ... args: Array<any> ) => any ) | Record<string, any> ) // not null or undefined
    : void {
...
function pushAdditionalFormField( this: Array<Chunk | PrecomputedChunk>, value: string | File, key: string ): void {
...
function pushAdditionalFormFields( target: Array<Chunk | PrecomputedChunk>, formData: null | FormData ) {
    if ( formData !== null ) {
...
function pushFormActionAttribute( target: Array<Chunk | PrecomputedChunk>, resumableState: ResumableState, renderState: RenderState, formAction: any, formEncType: any, formMethod: any, formTarget: any, name: any ): null | FormData {
    let formData = null;
...
function pushAttribute( target: Array<Chunk | PrecomputedChunk>, name: string, value: string | boolean | number | ( ( ... args: Array<any> ) => any ) | Record<string, any> ) // not null or undefined
    : void {
    switch ( name ) {
        ...
        case "span": {
            ...
            if ( typeof value !== "function" && typeof value !== "symbol" && ! isNaN( value ) && ( value as any ) >= 1 ) {
                target.push( attributeSeparator, stringToChunk( name ), attributeAssign, stringToChunk( escapeTextForBrowser( value ) ), attributeEnd );
            }
...
function pushInnerHTML( target: Array<Chunk | PrecomputedChunk>, innerHTML: any, children: any ) {
    if ( innerHTML != null ) {
        if ( children != null ) {
...
function pushStartSelect( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): ReactNodeList {
    if ( __DEV__ ) {
...
function pushStartOption( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, formatContext: FormatContext ): ReactNodeList {
...
const formReplayingRuntimeScript = stringToPrecomputedChunk( formReplayingRuntime );

function injectFormReplayingRuntime( resumableState: ResumableState, renderState: RenderState ): void {
...
export function pushFormStateMarkerIsMatching( target: Array<Chunk | PrecomputedChunk> ) {
    target.push( formStateMarkerIsMatching );
}

export function pushFormStateMarkerIsNotMatching( target: Array<Chunk | PrecomputedChunk> ) {
    target.push( formStateMarkerIsNotMatching );
}

function pushStartForm( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, resumableState: ResumableState, renderState: RenderState ): ReactNodeList {
...
function pushInput( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, resumableState: ResumableState, renderState: RenderState ): ReactNodeList {
    if ( __DEV__ ) {
...
function pushStartButton( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, resumableState: ResumableState, renderState: RenderState ): ReactNodeList {
...
function pushStartTextArea( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): ReactNodeList {
    if ( __DEV__ ) {
...
function pushMeta( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, renderState: RenderState, textEmbedded: boolean, insertionMode: InsertionMode, noscriptTagInScope: boolean ): null {
    if ( enableFloat ) {
...
function pushLink( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, resumableState: ResumableState, renderState: RenderState, textEmbedded: boolean, insertionMode: InsertionMode, noscriptTagInScope: boolean ): null {
    if ( enableFloat ) {
        const rel = props.rel;
...
function pushLinkImpl( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): null {
...
function pushStyle( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, resumableState: ResumableState, renderState: RenderState, textEmbedded: boolean, insertionMode: InsertionMode, noscriptTagInScope: boolean ): ReactNodeList {
    if ( __DEV__ ) {
...
function pushStyleImpl( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): ReactNodeList {
...
function pushStyleContents( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): void {
    let children = null;
    let innerHTML = null;
...
function pushImg( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, resumableState: ResumableState, renderState: RenderState, pictureTagInScope: boolean ): null {
    const {
        src,
        srcSet
    } = props;
...
function pushSelfClosing( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, tag: string ): null {
...
function pushStartMenuItem( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): ReactNodeList {
...
function pushTitle( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, renderState: RenderState, insertionMode: InsertionMode, noscriptTagInScope: boolean ): ReactNodeList {
    if ( __DEV__ ) {
        if ( hasOwnProperty.call( props, "children" ) ) {
            const children = props.children;
            const child = Array.isArray( children ) ? children.length < 2 ? children[ 0 ] : null : children;
...
function pushTitleImpl( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): null {
...
function pushStartTitle( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): ReactNodeList {
    target.push( startChunkForTag( "title" ) );
    let children = null;
...
function pushStartHead( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, renderState: RenderState, insertionMode: InsertionMode ): ReactNodeList {
    if ( enableFloat ) {
...
function pushStartHtml( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, renderState: RenderState, insertionMode: InsertionMode ): ReactNodeList {
    if ( enableFloat ) {
...
function pushScript( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, resumableState: ResumableState, renderState: RenderState, textEmbedded: boolean, insertionMode: InsertionMode, noscriptTagInScope: boolean ): null {
    if ( enableFloat ) {
...
function pushScriptImpl( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any> ): null {
...
function pushStartGenericElement( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, tag: string ): ReactNodeList {
    target.push( startChunkForTag( tag ) );
    let children = null;
    let innerHTML = null;
...
function pushStartCustomElement( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, tag: string ): ReactNodeList {
    target.push( startChunkForTag( tag ) );
    let children = null;
    let innerHTML = null;
...
function pushStartPreformattedElement( target: Array<Chunk | PrecomputedChunk>, props: Record<string, any>, tag: string ): ReactNodeList {
    target.push( startChunkForTag( tag ) );
    let children = null;
    let innerHTML = null;
...
export const doctypeChunk: PrecomputedChunk = stringToPrecomputedChunk( "<!DOCTYPE html>" );

export function pushStartInstance( target: Array<Chunk | PrecomputedChunk>, type: string, props: Record<string, any>, resumableState: ResumableState, renderState: RenderState, formatContext: FormatContext, textEmbedded: boolean ): ReactNodeList {
    if ( __DEV__ ) {
    ...
    switch ( type ) {
        ...
        case "title":
            return enableFloat ? pushTitle( target, props, renderState, formatContext.insertionMode, !! ( formatContext.tagScope & NOSCRIPT_SCOPE ) ) : pushStartTitle( target, props );
        ...
        case "script":
            return enableFloat ? pushScript( target, props, resumableState, renderState, textEmbedded, formatContext.insertionMode, !! ( formatContext.tagScope & NOSCRIPT_SCOPE ) ) : pushStartGenericElement( target, props, type );
        ...
        case "img": {
            return enableFloat ? pushImg( target, props, resumableState, renderState, !! ( formatContext.tagScope & PICTURE_SCOPE ) ) : pushSelfClosing( target, props, type );
        }
...
export function pushEndInstance( target: Array<Chunk | PrecomputedChunk>, type: string, props: Record<string, any>, resumableState: ResumableState, formatContext: FormatContext ): void {
    switch ( type ) {
...
function writeBootstrap( destination: Destination, renderState: RenderState ): boolean {
...
export function writeCompletedRoot( destination: Destination, renderState: RenderState ): boolean {
...
export function writePlaceholder( destination: Destination, renderState: RenderState, id: number ): boolean {
...
export function pushStartCompletedSuspenseBoundary( target: Array<Chunk | PrecomputedChunk> ) {
...
export function writeStartPendingSuspenseBoundary( destination: Destination, renderState: RenderState, id: number ): boolean {
...
export function writeStartClientRenderedSuspenseBoundary( destination: Destination, renderState: RenderState, errorDigest: string | null | undefined, errorMesssage: string | null | undefined, errorComponentStack: string | null | undefined ): boolean {
    let result;
...
export function writeEndPendingSuspenseBoundary( destination: Destination, renderState: RenderState ): boolean {
    return writeChunkAndReturn( destination, endSuspenseBoundary );
}
...
export function writeStartSegment( destination: Destination, renderState: RenderState, formatContext: FormatContext, id: number ): boolean {
...
export function writeEndSegment( destination: Destination, formatContext: FormatContext ): boolean {
    switch ( formatContext.insertionMode ) {
        case ROOT_HTML_MODE:
        case HTML_HTML_MODE:
        case HTML_MODE: {
...
export function writeCompletedSegmentInstruction( destination: Destination, resumableState: ResumableState, renderState: RenderState, contentSegmentID: number ): boolean {
    const scriptFormat = ! enableFizzExternalRuntime || resumableState.streamingFormat === ScriptStreamingFormat;
...
export function writeCompletedBoundaryInstruction( destination: Destination, resumableState: ResumableState, renderState: RenderState, id: number, boundaryResources: BoundaryResources ): boolean {
...
export function writeClientRenderBoundaryInstruction( destination: Destination, resumableState: ResumableState, renderState: RenderState, id: number, errorDigest: string | null | undefined, errorMessage?: string, errorComponentStack?: string ): boolean {
    const scriptFormat = ! enableFizzExternalRuntime || resumableState.streamingFormat === ScriptStreamingFormat;
...
function escapeJSObjectForInstructionScripts( input: Record<string, any> ): string {
    const escaped = JSON.stringify( input );
...
function flushStyleTagsLateForBoundary( this: Destination, styleQueue: StyleQueue ) {
...
export function writeResourcesForBoundary( destination: Destination, boundaryResources: BoundaryResources, renderState: RenderState ): boolean {
...
function flushStyleInPreamble( this: Destination, stylesheet: StylesheetResource, key: string, map: Map<string, StylesheetResource> ) {
...
function flushStylesInPreamble( this: Destination, styleQueue: StyleQueue, precedence: string ) {
...
export function writePreamble( destination: Destination, resumableState: ResumableState, renderState: RenderState, willFlushAllSegments: boolean ): void {
    // This function must be called exactly once on every request
    if ( enableFizzExternalRuntime && ! willFlushAllSegments && renderState.externalRuntimeScript ) {
...
// in the future to be backpressure sensitive but that requires a larger refactor
// of the flushing code in Fizz.
export function writeHoistables( destination: Destination, resumableState: ResumableState, renderState: RenderState ): void {
    let i = 0;
    // Emit high priority Hoistables
...
export function writePostamble( destination: Destination, resumableState: ResumableState ): void {
...
function writeStyleResourceDependenciesInJS( destination: Destination, boundaryResources: BoundaryResources ): void {
...
/* Helper functions */
function writeStyleResourceDependencyHrefOnlyInJS( destination: Destination, href: string ) {
...
function writeStyleResourceDependencyInJS( destination: Destination, href: unknown, precedence: unknown, props: Record<string, any> ) {
    // eslint-disable-next-line react-internal/safe-string-coercion
...
function writeStyleResourceAttributeInJS( destination: Destination, name: string, value: string | boolean | number | ( ( ... args: Array<any> ) => any ) | Record<string, any> ) // not null or undefined
    : void {
...
function writeStyleResourceDependenciesInAttr( destination: Destination, boundaryResources: BoundaryResources ): void {
    writeChunk( destination, arrayFirstOpenBracket );
    let nextArrayOpenBrackChunk = arrayFirstOpenBracket;
    boundaryResources.stylesheets.forEach( resource => {
        if ( resource.state === PREAMBLE ) {// We can elide this dependency because it was flushed in the shell and
...
/* Helper functions */
function writeStyleResourceDependencyHrefOnlyInAttr( destination: Destination, href: string ) {
    // We should actually enforce this earlier when the resource is created but for
    // now we make sure we are actually dealing with a string here.
    if ( __DEV__ ) {
        checkAttributeStringCoercion( href, "href" );
...
function writeStyleResourceDependencyInAttr( destination: Destination, href: unknown, precedence: unknown, props: Record<string, any> ) {
    // eslint-disable-next-line react-internal/safe-string-coercion
    const coercedHref = sanitizeURL( "" + ( href as any ) );
    writeChunk( destination, stringToChunk( escapeTextForBrowser( JSON.stringify( coercedHref ) ) ) );
...
function writeStyleResourceAttributeInAttr( destination: Destination, name: string, value: string | boolean | number | ( ( ... args: Array<any> ) => any ) | Record<string, any> ) // not null or undefined
    : void {
    let attributeName = name.toLowerCase();
    let attributeValue;
...
export function setCurrentlyRenderingBoundaryResourcesTarget( renderState: RenderState, boundaryResources: null | BoundaryResources ) {
...
function getImageResourceKey( href: string, imageSrcSet?: string | null | undefined, imageSizes?: string | null | undefined ): string {
    if ( imageSrcSet ) {
        return imageSrcSet + "\n" + ( imageSizes || "" );
    }
...
function preconnect( href: string, crossOrigin: CrossOriginEnum | null | undefined ) {
    ...
    if ( typeof href === "string" && href ) {
        const resources = crossOrigin === "use-credentials" ? resumableState.connectResources.credentials : typeof crossOrigin === "string" ? resumableState.connectResources.anonymous : resumableState.connectResources.default;
        const key = getResourceKey( href );
...
function preloadModule( href: string, options?: PreloadModuleImplOptions | null | undefined ): void {
    if ( ! enableFloat ) {
        return;
    }
...
function preinitStyle( href: string, precedence: string | null | undefined, options?: PreinitStyleOptions | null | undefined ): void {
    if ( ! enableFloat ) {
        return;
    }
...
function preinitScript( src: string, options?: PreinitScriptOptions | null | undefined ): void {
    if ( ! enableFloat ) {
        return;
    }
...
function preinitModuleScript( src: string, options?: PreinitModuleScriptOptions | null | undefined ): void {
    if ( ! enableFloat ) {
        return;
    }
...
function preloadBootstrapScriptOrModule( resumableState: ResumableState, renderState: RenderState, href: string, props: PreloadProps ): void {
    if ( ! enableFloat ) {
        return;
    }
...
function internalPreinitScript( resumableState: ResumableState, renderState: RenderState, src: string, chunks: Array<Chunk | PrecomputedChunk> ): void {
    const key = getResourceKey( src );
...
function adoptPreloadCredentials( target: StylesheetProps | ScriptProps | ModuleScriptProps, preloadState: PreloadedWithCredentials ): void {
    if ( target.crossOrigin == null ) target.crossOrigin = preloadState[ 0 ];
    if ( target.integrity == null ) target.integrity = preloadState[ 1 ];
}

function hoistStyleQueueDependency( this: BoundaryResources, styleQueue: StyleQueue ) {
    this.styles.add( styleQueue );
}

function hoistStylesheetDependency( this: BoundaryResources, stylesheet: StylesheetResource ) {
    this.stylesheets.add( stylesheet );
}

export function hoistResources( renderState: RenderState, source: BoundaryResources ): void {
