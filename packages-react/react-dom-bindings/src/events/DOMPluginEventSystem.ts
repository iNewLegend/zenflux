...
import { invokeGuardedCallbackAndCatchFirstError, rethrowCaughtError } from "@zenflux/react-shared/src/react-error-utils";
...
import { COMMENT_NODE, DOCUMENT_NODE } from "@zenflux/react-dom-bindings/src/client/HTMLNodeType";
...
import { createEventListenerWrapperWithPriority } from "@zenflux/react-dom-bindings/src/events/ReactDOMEventListener";
import {
...
import type {
    ...
    ReactSyntheticEvent
} from "@zenflux/react-dom-bindings/src/events/ReactSyntheticEventType";
...
function deferClickToDocumentForLegacyFBSupport( domEventName: DOMEventName, targetContainer: EventTarget ): void {
            ...
            mainLoop: while ( true ) {
                ...
                if ( nodeTag === HostRoot || nodeTag === HostPortal ) {
                    ...
                    while ( container !== null ) {
                        ...
                        if ( parentTag === HostComponent || parentTag === HostText || ( enableFloat ? parentTag === HostHoistable : false ) || ( enableHostSingletons ? parentTag === HostSingleton : false ) ) {
...
function createDispatchListener( instance: null | Fiber, listener: ( ... args: Array<any> ) => any, currentTarget: EventTarget ): DispatchListener {
    return {
        instance,
        listener,
        currentTarget
...
export function accumulateSinglePhaseListeners( targetFiber: Fiber | null, reactName: string | null, nativeEventType: string, inCapturePhase: boolean, accumulateTargetOnly: boolean, nativeEvent: AnyNativeEvent ): Array<DispatchListener> {
    ...
    while ( instance !== null ) {
        ...
        if ( ( tag === HostComponent || ( enableFloat ? tag === HostHoistable : false ) || ( enableHostSingletons ? tag === HostSingleton : false ) ) && stateNode !== null ) {
