...
import { accumulateTwoPhaseListeners, processDispatchQueue } from "@zenflux/react-dom-bindings/src/events/DOMPluginEventSystem";
...
import isEventSupported from "@zenflux/react-dom-bindings/src/events/isEventSupported";
import { getNodeFromInstance } from "@zenflux/react-dom-bindings/src/client/ReactDOMComponentTree";
import { updateValueIfChanged } from "@zenflux/react-dom-bindings/src/client/inputValueTracking";
import { setDefaultValue } from "@zenflux/react-dom-bindings/src/client/ReactDOMInput";
import { enqueueStateRestore } from "@zenflux/react-dom-bindings/src/events/ReactDOMControlledComponent";
...
function createAndAccumulateChangeEvent( dispatchQueue: DispatchQueue, inst: null | Fiber, nativeEvent: AnyNativeEvent, target: null | EventTarget ) {
    ...
    if ( listeners.length > 0 ) {
        const event: ReactSyntheticEvent = new SyntheticEvent( "onChange", "change", null, nativeEvent, target );
        dispatchQueue.push( {
            event,
            listeners
        } );
 ...
 * For IE shims
 */
let activeElement: Instance | TextInstance | null = null;
...
function shouldUseChangeEvent( elem: Instance | TextInstance ) {
    ...
    return nodeName === "select" || nodeName === "input" && ( elem as any ).type === "file";
}
 ...
 */
function handlePropertyChange( nativeEvent: AnyNativeEvent & { propertyName: string } ) {
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function extractEvents( dispatchQueue: DispatchQueue, domEventName: DOMEventName, targetInst: null | Fiber, nativeEvent: AnyNativeEvent, nativeEventTarget: null | EventTarget, eventSystemFlags: EventSystemFlags, targetContainer: null | EventTarget ) {
    ...
    } else if ( enableCustomElementPropertySupport && targetInst && isCustomElement( targetInst.elementType, targetInst.memoizedProps ) ) {
        getTargetInstFunc = getTargetInstForChangeEvent;
    }
