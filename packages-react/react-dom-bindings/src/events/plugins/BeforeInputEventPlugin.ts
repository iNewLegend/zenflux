...
} from "@zenflux/react-dom-bindings/src/events/FallbackCompositionState";
import { SyntheticCompositionEvent, SyntheticInputEvent } from "@zenflux/react-dom-bindings/src/events/SyntheticEvent";

import { registerTwoPhaseEvent } from "@zenflux/react-dom-bindings/src/events/EventRegistry";

import { accumulateTwoPhaseListeners } from "@zenflux/react-dom-bindings/src/events/DOMPluginEventSystem";
 ...
 */
function isFallbackCompositionStart( domEventName: DOMEventName, nativeEvent: any ): boolean {
 ...
 */
function isFallbackCompositionEnd( domEventName: DOMEventName, nativeEvent: any ): boolean {
    switch ( domEventName ) {
        case "keyup":
...
function extractCompositionEvent( dispatchQueue: DispatchQueue, domEventName: DOMEventName, targetInst: null | Fiber, nativeEvent: AnyNativeEvent, nativeEventTarget: null | EventTarget ) {
    ...
    if ( listeners.length > 0 ) {
        const event: ReactSyntheticEvent = new SyntheticCompositionEvent( eventType, domEventName, null, nativeEvent, nativeEventTarget );
        dispatchQueue.push( {
            event,
            listeners
        } );
...
function getNativeBeforeInputChars( domEventName: DOMEventName, nativeEvent: any ): string | null | undefined {
    switch ( domEventName ) {
        case "compositionend":
 ...
 */
function getFallbackBeforeInputChars( domEventName: DOMEventName, nativeEvent: any ): string | null | undefined {
...
function extractBeforeInputEvent( dispatchQueue: DispatchQueue, domEventName: DOMEventName, targetInst: null | Fiber, nativeEvent: AnyNativeEvent, nativeEventTarget: null | EventTarget ) {
    ...
    if ( listeners.length > 0 ) {
        const event: ReactSyntheticEvent = new SyntheticInputEvent( "onBeforeInput", "beforeinput", null, nativeEvent, nativeEventTarget );
        dispatchQueue.push( {
            event,
            listeners
        } );
