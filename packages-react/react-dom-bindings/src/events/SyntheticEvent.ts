...
interface EventInterfaceType {
    deltaX?: number | ( ( event: Record<string, unknown> ) => unknown );
    deltaY?: number | ( ( event: Record<string, unknown> ) => unknown );
...
const WheelEventInterface: EventInterfaceType = {
    ...
    deltaX( event: Record<string, unknown> ) {
    ...
    deltaY( event: Record<string, unknown> ) {
...
function createSyntheticEvent( Interface: EventInterfaceType ) {
    ...
    function SyntheticBaseEvent( reactName: string | null, reactEventType: string, targetInst: Fiber | null, nativeEvent: AnyNativeEvent, nativeEventTarget: null | EventTarget ) {
        ...
        for ( const propName in Interface ) {
            ...
            if ( normalize ) {
                self[ propName ] = normalize( nativeEvent );
            } else {
                self[ propName ] = nativeEvent[ propName ];
            }
        }
