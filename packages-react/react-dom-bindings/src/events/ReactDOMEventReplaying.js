...
function attemptReplayContinuousQueuedEvent(queuedEvent) {
    ...
    while (targetContainers.length > 0) {
        var nextBlockedOn = (0, ReactDOMEventListener_1.findInstanceBlockingEvent)(queuedEvent.nativeEvent);
...
function replayUnblockedFormActions(formReplayingQueue) {
    ...
    for (var i_1 = 0; i_1 < formReplayingQueue.length; i_1 += 3) {
        var form = formReplayingQueue[i_1];
        var submitterOrAction = formReplayingQueue[i_1 + 1];
        var formData = formReplayingQueue[i_1 + 2];
        if (typeof submitterOrAction !== "function") {
            ...
            var blockedOn = (0, ReactDOMEventListener_1.findInstanceBlockingTarget)(submitterOrAction || form);
            if (blockedOn === null) {
        ...
        if (formInst !== null) {
            ...
            formReplayingQueue.splice(i_1, 3);
            i_1 -= 3;
...
function retryIfBlockedOn(unblocked) {
    ...
    for (var i_2 = 0; i_2 < queuedExplicitHydrationTargets.length; i_2++) {
        var queuedTarget = queuedExplicitHydrationTargets[i_2];
    ...
    if (react_feature_flags_1.enableFormActions) {
        ...
        if (formReplayingQueue != null) {
            for (var i_3 = 0; i_3 < formReplayingQueue.length; i_3 += 3) {
                var form = formReplayingQueue[i_3];
                var submitterOrAction = formReplayingQueue[i_3 + 1];
                ...
                if (formProps) {
                    ...
                    if (submitter && submitter.hasAttribute("formAction")) {
                        ...
                        if (submitterProps) {
                            ...
                            var blockedOn = (0, ReactDOMEventListener_1.findInstanceBlockingTarget)(target);
                            if (blockedOn !== null) {
                    ...
                    if (typeof action === "function") {
                        formReplayingQueue[i_3 + 1] = action;
                    }
                    else {
                        // Something went wrong so let's just delete this action.
                        formReplayingQueue.splice(i_3, 3);
                        i_3 -= 3;
                    }
