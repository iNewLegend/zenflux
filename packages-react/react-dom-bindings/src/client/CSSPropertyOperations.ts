 ...
 */
import { checkCSSPropertyStringCoercion } from "@zenflux/react-shared/src/check-string-coercion";

import hyphenateStyleName from "@zenflux/react-dom-bindings/src/shared/hyphenateStyleName";

import warnValidStyle from "@zenflux/react-dom-bindings/src/shared/warnValidStyle";
import isUnitlessNumber from "@zenflux/react-dom-bindings/src/shared/isUnitlessNumber";
 ...
 */
export function createDangerousStringForStyles( styles: { [ key: string ]: string } ) {
    if ( __DEV__ ) {
        let serialized = "";
        let delimiter = "";
...
function setValueForStyle( style: CSSStyleDeclaration, styleName: any, value: any ) {
    const isCustomProperty = styleName.indexOf( "--" ) === 0;
 ...
 * // @param {object} styles
 */
export function setValueForStyles( node: HTMLElement, styles: { [ key: string ]: string }, prevStyles: {
    [ key: string ]: string
} ) {
 ...
 */
function expandShorthandMap( styles: { [ key: string ]: string } ) {
    const expanded = {};
 ...
 */
function validateShorthandPropertyCollisionInDev( prevStyles: { [ key: string ]: string }, nextStyles: {
    [ key: string ]: string
} ) {
    if ( __DEV__ ) {
        if ( ! nextStyles ) {
