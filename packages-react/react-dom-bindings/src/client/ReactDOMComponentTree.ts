...
import { getParentSuspenseInstance } from "@zenflux/react-dom-bindings/src/client/ReactFiberConfigDOM";
...
export function getInstanceFromNode( node: Node ): Fiber | null {
    ...
    if ( inst ) {
        ...
        if ( tag === HostComponent || tag === HostText || tag === SuspenseComponent || ( enableFloat ? tag === HostHoistable : false ) || ( enableHostSingletons ? tag === HostSingleton : false ) || tag === HostRoot ) {
            return inst;
        } else {
            return null;
        }
