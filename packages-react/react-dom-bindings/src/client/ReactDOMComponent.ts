...
import { possibleRegistrationNames, registrationNameDependencies } from "@zenflux/react-dom-bindings/src/events/EventRegistry";

import { checkControlledValueProps } from "@zenflux/react-dom-bindings/src/shared/ReactControlledValuePropTypes";

import isCustomElement from "@zenflux/react-dom-bindings/src/shared/isCustomElement";

import getAttributeAlias from "@zenflux/react-dom-bindings/src/shared/getAttributeAlias";

import possibleStandardNames from "@zenflux/react-dom-bindings/src/shared/possibleStandardNames";

import { validateProperties as validateARIAProperties } from "@zenflux/react-dom-bindings/src/shared/ReactDOMInvalidARIAHook";

import { HostContextNamespaceNone } from "@zenflux/react-dom-bindings/src/client/ReactFiberConfigDOM";
...
} from "@zenflux/react-dom-bindings/src/client/DOMPropertyOperations";
import {
    initInput,
    restoreControlledInputState,
    updateInput,
    validateInputProps
} from "@zenflux/react-dom-bindings/src/client/ReactDOMInput";
import { validateOptionProps } from "@zenflux/react-dom-bindings/src/client/ReactDOMOption";
import {
    initSelect,
    restoreControlledSelectState,
    updateSelect,
    validateSelectProps
} from "@zenflux/react-dom-bindings/src/client/ReactDOMSelect";
import {
    initTextarea,
    restoreControlledTextareaState,
    updateTextarea,
    validateTextareaProps
} from "@zenflux/react-dom-bindings/src/client/ReactDOMTextarea";
import { validateTextNesting } from "@zenflux/react-dom-bindings/src/client/validateDOMNesting";
import { track } from "@zenflux/react-dom-bindings/src/client/inputValueTracking";
import setInnerHTML from "@zenflux/react-dom-bindings/src/client/setInnerHTML";
import setTextContent from "@zenflux/react-dom-bindings/src/client/setTextContent";
import { createDangerousStringForStyles, setValueForStyles } from "@zenflux/react-dom-bindings/src/client/CSSPropertyOperations";
...
} from "@zenflux/react-dom-bindings/src/shared/ReactDOMNullInputValuePropHook";
import {
    validateProperties as validateUnknownProperties
} from "@zenflux/react-dom-bindings/src/shared/ReactDOMUnknownPropertyHook";
import sanitizeURL from "@zenflux/react-dom-bindings/src/shared/sanitizeURL";

import { listenToNonDelegatedEvent, mediaEventTypes } from "@zenflux/react-dom-bindings/src/events/DOMPluginEventSystem";

import type { HostContext, HostContextDev } from "@zenflux/react-dom-bindings/src/client/ReactFiberConfigDOM";
...
function validateFormActionInDevelopment( tag: string, key: string, value: unknown, props: any ) {
    if ( __DEV__ ) {
        if ( value == null ) {
            return;
        }
...
function warnForPropDifference( propName: string, serverValue: unknown, clientValue: unknown ) {
    if ( __DEV__ ) {
...
function normalizeHTML( parent: Element, html: string ) {
    if ( __DEV__ ) {
        ...
        const testElement = parent.namespaceURI === MATH_NAMESPACE || parent.namespaceURI === SVG_NAMESPACE ? parent.ownerDocument.createElementNS( ( parent.namespaceURI as any ), parent.tagName ) : parent.ownerDocument.createElement( parent.tagName );
...
function normalizeMarkupForTextOrAttribute( markup: unknown ): string {
    if ( __DEV__ ) {
...
export function checkForUnmatchedText( serverText: string | null, clientText: string | number, isConcurrentMode: boolean, shouldWarnDev: boolean ) {
...
function setProp( domElement: Element, tag: string, key: string, value: any, props: any, prevValue: any ): void {
    switch ( key ) {
        case "children": {
            if ( typeof value === "string" ) {
                ...
                const canSetTextContent = ( ! enableHostSingletons || tag !== "body" ) && ( tag !== "textarea" || value !== "" );
        ...
        case "href": {
            ...
            const sanitizedValue = ( sanitizeURL( enableTrustedTypesIntegration ? value : "" + ( value as any ) ) as any );
        ...
        case "formAction": {
            ...
            if ( value == null || ! enableFormActions && typeof value === "function" || typeof value === "symbol" || typeof value === "boolean" ) {
                domElement.removeAttribute( key );
                break;
            }
        ...
        case "multiple": {
            ( domElement as any ).multiple = value && typeof value !== "function" && typeof value !== "symbol";
            break;
        }

        case "muted": {
            ( domElement as any ).muted = value && typeof value !== "function" && typeof value !== "symbol";
            break;
        }
        ...
        case "preserveAlpha": {
            ...
            if ( value != null && typeof value !== "function" && typeof value !== "symbol" ) {
                ...
                domElement.setAttribute( key, enableTrustedTypesIntegration ? ( value as any ) : "" + ( value as any ) );
            } else {
                domElement.removeAttribute( key );
            }
        ...
        case "span": {
            ...
            if ( value != null && typeof value !== "function" && typeof value !== "symbol" && ! isNaN( value ) && ( value as any ) >= 1 ) {
                if ( __DEV__ ) {
                    checkAttributeStringCoercion( value, key );
                }
        ...
        default: {
            if ( key.length > 2 && ( key[ 0 ] === "o" || key[ 0 ] === "O" ) && ( key[ 1 ] === "n" || key[ 1 ] === "N" ) ) {
                if ( __DEV__ && registrationNameDependencies.hasOwnProperty( key ) && value != null && typeof value !== "function" ) {
                    warnForInvalidEventListener( key, value );
                }
            } else {
...
function setPropOnCustomElement( domElement: Element, tag: string, key: string, value: any, props: any, prevValue: any ): void {
    switch ( key ) {
        case "style": {
            setValueForStyles( domElement as HTMLElement, value, prevValue );
            break;
...
export function setInitialProperties( domElement: Element, tag: string, props: Record<string, any> ): void {
    if ( __DEV__ ) {
            ...
            for ( const propKey in props ) {
                ...
                switch ( propKey ) {
                    case "selected": {
                        ...
                        ( domElement as any ).selected = propValue && typeof propValue !== "function" && typeof propValue !== "symbol";
                        break;
                    }
...
export function updateProperties( domElement: Element, tag: string, lastProps: Record<string, any>, nextProps: Record<string, any> ): void {
    if ( __DEV__ ) {
            ...
            for ( const propKey in nextProps ) {
                ...
                if ( nextProps.hasOwnProperty( propKey ) && nextProp !== lastProp && ( nextProp != null || lastProp != null ) ) {
                    switch ( propKey ) {
                        case "selected": {
                            // TODO: Remove support for selected on option.
                            ( domElement as any ).selected = nextProp && typeof nextProp !== "function" && typeof nextProp !== "symbol";
                            break;
                        }
...
function diffHydratedStyles( domElement: Element, value: any ) {
...
function hydrateAttribute( domElement: Element, propKey: string, attributeName: string, value: any, extraAttributes: Set<string> ): void {
...
function hydrateBooleanAttribute( domElement: Element, propKey: string, attributeName: string, value: any, extraAttributes: Set<string> ): void {
    extraAttributes.delete( attributeName );
    const serverValue = domElement.getAttribute( attributeName );
...
function hydrateOverloadedBooleanAttribute( domElement: Element, propKey: string, attributeName: string, value: any, extraAttributes: Set<string> ): void {
    extraAttributes.delete( attributeName );
    const serverValue = domElement.getAttribute( attributeName );
...
function hydrateBooleanishAttribute( domElement: Element, propKey: string, attributeName: string, value: any, extraAttributes: Set<string> ): void {
    extraAttributes.delete( attributeName );
    const serverValue = domElement.getAttribute( attributeName );
...
function hydrateNumericAttribute( domElement: Element, propKey: string, attributeName: string, value: any, extraAttributes: Set<string> ): void {
    extraAttributes.delete( attributeName );
    const serverValue = domElement.getAttribute( attributeName );
...
function hydratePositiveNumericAttribute( domElement: Element, propKey: string, attributeName: string, value: any, extraAttributes: Set<string> ): void {
    extraAttributes.delete( attributeName );
    const serverValue = domElement.getAttribute( attributeName );
...
function hydrateSanitizedAttribute( domElement: Element, propKey: string, attributeName: string, value: any, extraAttributes: Set<string> ): void {
    extraAttributes.delete( attributeName );
    const serverValue = domElement.getAttribute( attributeName );
...
function diffHydratedCustomComponent( domElement: Element, tag: string, props: Record<string, any>, hostContext: HostContext, extraAttributes: Set<string> ) {
    for ( const propKey in props ) {
        if ( ! props.hasOwnProperty( propKey ) ) {
            continue;
        }
        ...
        switch ( propKey ) {
            ...
            default: {
                ...
                const serverValue = getValueForAttributeOnCustomComponent( domElement, propKey, value );
                warnForPropDifference( propKey, serverValue, value );
            }
        }
    }
...
export function diffHydratedProperties( domElement: Element, tag: string, props: Record<string, any>, isConcurrentMode: boolean, shouldWarnDev: boolean, hostContext: HostContext ): void {
    if ( __DEV__ ) {
        validatePropertiesInDevelopment( tag, props );
    }
        ...
        if ( domElement.textContent !== "" + children ) {
            ...
            if ( ! isConcurrentMode || ! enableClientRenderFallbackOnTextMismatch ) {
                ...
                if ( ! enableHostSingletons || tag !== "body" ) {
                    domElement.textContent = ( children as any );
                }
            }
        }
    }
...
export function warnForDeletedHydratableElement( parentNode: Element | Document | DocumentFragment, child: Element ) {
    if ( __DEV__ ) {
        if ( didWarnInvalidHydration ) {
            return;
        }
...
export function warnForDeletedHydratableText( parentNode: Element | Document | DocumentFragment, child: Text ) {
    if ( __DEV__ ) {
        if ( didWarnInvalidHydration ) {
            return;
        }
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export function warnForInsertedHydratedElement( parentNode: Element | Document | DocumentFragment, tag: string, props: Record<string, any> ) {
    if ( __DEV__ ) {
        if ( didWarnInvalidHydration ) {
            return;
        }
...
export function warnForInsertedHydratedText( parentNode: Element | Document | DocumentFragment, text: string ) {
    if ( __DEV__ ) {
        if ( text === "" ) {
