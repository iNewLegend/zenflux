        ...
        default: {
            if (EventRegistry_1.registrationNameDependencies.hasOwnProperty(key)) {
                ...
                if (react_feature_flags_1.enableCustomElementPropertySupport) {
                    ...
                    (0, DOMPropertyOperations_1.setValueForAttribute)(domElement, key, value);
                }
            }
        }
    }
...
function setInitialProperties(domElement, tag, props) {
    ...
    switch (tag) {
        ...
        case "audio": {
            ...
            for (var i_1 = 0; i_1 < DOMPluginEventSystem_1.mediaEventTypes.length; i_1++) {
                (0, DOMPluginEventSystem_1.listenToNonDelegatedEvent)(DOMPluginEventSystem_1.mediaEventTypes[i_1], domElement);
            }
            break;
        }
        case "image": {
    ...
    for (var propKey in props) {
        ...
        switch (propKey) {
            ...
            default: {
                ...
                if (hostContextProd === ReactFiberConfigDOM_1.HostContextNamespaceNone && tag !== "svg" && tag !== "math") {
                    extraAttributes.delete(propKey.toLowerCase());
                }
                else {
                    extraAttributes.delete(propKey);
...
function diffHydratedProperties(domElement, tag, props, isConcurrentMode, shouldWarnDev, hostContext) {
    ...
    switch (tag) {
        ...
        case "audio":
            // We listen to these events in case to ensure emulated bubble
            // listeners still fire for all the media events.
            for (var i_2 = 0; i_2 < DOMPluginEventSystem_1.mediaEventTypes.length; i_2++) {
                (0, DOMPluginEventSystem_1.listenToNonDelegatedEvent)(DOMPluginEventSystem_1.mediaEventTypes[i_2], domElement);
            }
            break;
        case "source":
            // We listen to this event in case to ensure emulated bubble
    ...
    if (__DEV__ && shouldWarnDev) {
        ...
        for (var i_3 = 0; i_3 < attributes.length; i_3++) {
            var name_3 = attributes[i_3].name.toLowerCase();
            switch (name_3) {
                ...
                default:
                    ...
                    extraAttributes.add(attributes[i_3].name);
            }
        }
        if ((0, isCustomElement_1.default)(tag, props)) {
...
function restoreControlledState(domElement, tag, props) {
    switch (tag) {
        case "input":
            ...
            return;
        case "textarea":
            (0, ReactDOMTextarea_1.restoreControlledTextareaState)(domElement, props);
            return;
        case "select":
