    ...
    while (parentNode) {
        ...
        if (targetInst) {
            ...
            if (targetInst.child !== null || alternate !== null && alternate.child !== null) {
                ...
                var suspenseInstance = (0, ReactFiberConfigDOM_1.getParentSuspenseInstance)(targetNode);
                while (suspenseInstance !== null) {
                    ...
                    suspenseInstance = (0, ReactFiberConfigDOM_1.getParentSuspenseInstance)(suspenseInstance); // If we don't find one, then that should mean that the parent
...
function getInstanceFromNode(node) {
    ...
    if (inst) {
        var tag = inst.tag;
        if (tag === react_work_tags_1.HostComponent || tag === react_work_tags_1.HostText || tag === react_work_tags_1.SuspenseComponent || (react_feature_flags_1.enableFloat ? tag === react_work_tags_1.HostHoistable : false) || (react_feature_flags_1.enableHostSingletons ? tag === react_work_tags_1.HostSingleton : false) || tag === react_work_tags_1.HostRoot) {
            return inst;
        }
        else {
            return null;
...
function getNodeFromInstance(inst) {
    ...
    var tag = inst.tag;
    if (tag === react_work_tags_1.HostComponent || (react_feature_flags_1.enableFloat ? tag === react_work_tags_1.HostHoistable : false) || (react_feature_flags_1.enableHostSingletons ? tag === react_work_tags_1.HostSingleton : false) || tag === react_work_tags_1.HostText) {
