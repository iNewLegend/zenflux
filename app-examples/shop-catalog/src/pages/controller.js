"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.PagesController = void 0;
var core_1 = require("@zenflux/core");
var redux_1 = require("@zenflux/redux");
var PagesController = /** @class */ (function (_super) {
    __extends(PagesController, _super);
    function PagesController() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    PagesController.getName = function () {
        return "Pages/Controller";
    };
    PagesController.prototype.getRoutes = function () {
        return {
            "Catalog": function () {
                var catalogController = core_1.default.managers.controllers.get("Catalog/Controller");
                catalogController.getCatalog();
            },
            "Checkout": function () { },
        };
    };
    return PagesController;
}(redux_1.default.core.RouterController));
exports.PagesController = PagesController;
exports.default = PagesController;
